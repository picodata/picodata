...
pub struct RaftMessageExt {
    /// Info generated by raft-rs as part of the implementation of the raft
    /// algorithm.
    inner: raft::Message,
    ...
    // TODO: make this mandatory in next major release
...
impl RaftMessageExt {
    ...
    #[inline(always)]
    fn new(inner: raft::Message, applied: RaftIndex) -> Self {
        Self {
            inner,
    ...
    fn decode(data: &[u8]) -> Result<Self> {
        ...
        let mut inner = raft::Message::default();
        inner
    ...
    fn encode(&self, buffer: &mut impl std::io::Write) -> Result<(), BoxError> {
        rmp::encode::write_array_len(buffer, 3).map_err(to_error_other)?;
        ...
        let applied = self.applied.expect("we always add this");
        rmp::encode::write_uint(buffer, applied).map_err(to_error_other)?;
