...
use crate::sync_replication::get_tier_replication_mode_and_factor;
use crate::tlog;
...
impl Cfg {
    ...
    pub fn for_instance_join(
        ...
        let mut res = Self {
            ...
            // work around `ER_BOOTSTRAP_CONNECTION_NOT_TO_ALL` error,
            // which is triggered only for `bootstrap_strategy = auto`.
            // Since Picodata manages its own bootstrap flow
            // (it knows the master, controls replication topology),
            // the auto strategy's check is unnecessary.
