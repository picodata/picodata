...
impl TarantoolBuildRoot {
    /// Read `box-include-args` and `box-define-args` files generated by tarantool CMake to determine
    /// the include and define flags that box was compiled with.
    pub fn get_box_flags(&self) -> Vec<String> {
        let tarantool_build = self.tarantool_build_dir();

        let include_args_str =
            std::fs::read_to_string(tarantool_build.join("box-include-args")).unwrap();
        let define_args_str =
            std::fs::read_to_string(tarantool_build.join("box-define-args")).unwrap();

        // This is not robust against spaces in the paths and will split them into separate args.
        // Unfortunately, there is no easy way to reliably write a CMake array to a file without:
        // - succumbing to "there are certain forbidden characters" limitation
        // - bumping the required version of CMake
        // The upside is that tarantool won't be able to build in a directory with spaces in the name anyways,
        // so this is not changing the status quo.
        include_args_str
            .split(' ')
            .chain(define_args_str.split(' '))
            .map(String::from)
            .collect()
    }
}
