...
use crate::ir::node::{Row, UnaryExpr};
    ...
    pub fn fold_boolean_tree(&mut self) -> Result<(), SbroadError> {
        ...
        for LevelNode(_, id) in nodes.iter() {
                ...
                match self.get_mut_expression_node(*op_id)? {
                    MutExpression::Bool(BoolExpr { left, op, right }) => {
                        ...
                        *right = *const_map.get(right).unwrap_or(right);
                        ...
                        let (left_node, right_node) = if let (
                            Expression::Row(Row { list: lhs_list, .. }),
                            Expression::Row(Row { list: rhs_list, .. }),
                        ) = (
                            self.get_expression_node(left)?,
                            self.get_expression_node(right)?,
                        ) {
                            if lhs_list.len() != 1 {
                                continue;
                            }
                            ...
                            (
                                self.get_expression_node(*lhs_list.first().unwrap())?,
                                self.get_expression_node(*rhs_list.first().unwrap())?,
                            )
                        } else {
                            (
                                self.get_expression_node(left)?,
                        ...
                        ) = (left_node, right_node)
                        {
                            ...
                            let const_id = self.nodes.add_const(val);
                            let row_id =
                                self.nodes.add_row(vec![const_id], Some(Distribution::Any));
                            const_map.insert(*op_id, row_id);
                        }
                    }
                    MutExpression::Unary(UnaryExpr { op, child }) => {
                        *child = *const_map.get(child).unwrap_or(child);
                        ...
                        let child_node = if let Expression::Row(Row { list, .. }) =
                            self.get_expression_node(child)?
                        {
                            if list.len() != 1 {
                                continue;
                            }
                            self.get_expression_node(*list.first().unwrap())?
                        } else {
                            self.get_expression_node(child)?
                        };
            ...
            if let Some(new_filter) = const_map.get(&filter) {
                let child_row_node = self.get_expression_node(*new_filter)?;
                let Expression::Row(Row { list, .. }) = child_row_node else {
                    unreachable!("expected Row node!");
                };
                if list.len() != 1 {
                    continue;
                }

                let const_node_id = *list.first().unwrap();
