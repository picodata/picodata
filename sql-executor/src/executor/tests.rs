...
fn shard_query() {
    ...
    let DispatchInfo::Filtered(filtered) = info.get(0).unwrap() else {
        panic!("Expected a custom plan dispatch");
    };
...
fn linker_test() {
    ...
    assert_eq!(1, info.len());
    let DispatchInfo::Filtered(ref mut filtered) = info.get_mut(0).unwrap() else {
        panic!("Expected a custom plan dispatch");
    };
    assert_eq!(1, filtered.len());
    ...
    let bucket2 = query
        ...
        .unwrap();
    let mut expected = vec![bucket2, bucket3];
...
fn join_linker4_test() {
    ...
    if let MotionPolicy::Segment(key) =
        get_motion_policy(query.get_exec_plan().get_ir_plan(), motion_t2_id)
    {
    ...
    if let MotionPolicy::Segment(key) =
        get_motion_policy(query.get_exec_plan().get_ir_plan(), motion_sq_id)
    {
        virtual_sq.reshard(key, query.get_coordinator()).unwrap();
    }
    query
        .get_coordinator()
    ...
    let mut info = port.decode();
    assert_eq!(1, info.len());
    let DispatchInfo::Filtered(ref mut filtered) = info.first_mut().unwrap() else {
        panic!("Expected a custom plan dispatch");
    };
    assert_eq!(1, filtered.len());
    ...
    let bucket1 = query
        ...
        .determine_bucket_id(&[&param1])
        .unwrap();
    let mut expected = vec![bucket1, bucket2];
    buckets.sort_unstable();
    expected.sort_unstable();
    assert_eq!(buckets, &expected);
}
...
fn anonymous_col_index_test() {
    ...
    assert_eq!(1, info.len());
    let DispatchInfo::Filtered(ref mut filtered) = info.first_mut().unwrap() else {
        panic!("Expected filtered dispatch");
    };
    assert_eq!(1, filtered.len());
    let (sql, params, _, ref mut buckets) = filtered.first_mut().unwrap();
...
fn groupby_linker_test() {
    ...
    query
        .get_coordinator()
        .add_virtual_table(motion_id, virtual_t1);
