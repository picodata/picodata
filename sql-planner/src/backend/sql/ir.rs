...
impl ExecutionPlan {
    ...
    pub fn calculate_plan_id(&self, top_id: NodeId) -> Result<u64, SbroadError> {
        let hash = {
            let constants = self.get_ir_plan().get_const_list(top_id, Snapshot::Oldest);
            let sp = SyntaxPlan::new(self, top_id, Snapshot::Oldest, true)?;
            let ordered = OrderedSyntaxNodes::try_from(sp)?;
            let nodes = ordered.to_syntax_data()?;
            let sql = self.generate_sql(&nodes, 0, table_name, Some(constants))?;
            ...
            self.get_vtables()
                ...
                .flat_map(|(_, vtable)| vtable.get_columns())
                .for_each(|column| {
                    hasher.write(column.name.as_bytes());
                    hasher.write_u8(column.r#type.into());
                });
    ...
    pub fn get_plan_id(&self) -> Result<u64, SbroadError> {
        ...
        hasher.write_usize(self.get_ir_plan().constants.len());
        let plan_id = hasher.finish();

        Ok(plan_id)
    }

    /// If constants were stashed, `constans` must be equal `None`.
    pub fn generate_sql(
        &self,
        nodes: &[impl AsRef<SyntaxData>],
        plan_id: u64,
        table_name: fn(u64, NodeId) -> SmolStr,
        constants: Option<Vec<NodeId>>,
        ...
        let constants = constants.map(|constants| {
            constants
                .into_iter()
                .enumerate()
                .map(|(num, id)| (id, num + 1))
        ...
        let constants_count = constants
            .as_ref()
            .map_or_else(|| ir_plan.constants.len(), |constants| constants.len());
