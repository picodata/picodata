...
use sql::ir::transformation::equality_facts::Slot;
...
fn equality_facts_1() {
    ...
    let equalities_facts = plan.facts.as_ref().unwrap();
...
fn find_left_join(plan: &Plan) -> (NodeId, NodeId, NodeId) {
    ...
    use sql::ir::node::{Join, Projection, Selection};
    use sql::ir::operator::JoinKind;
    let mut current = plan.get_top().unwrap();
    loop {
