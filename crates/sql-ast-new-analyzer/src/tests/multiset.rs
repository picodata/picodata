//! The WITH clause, and the `UNION` / `EXCEPT` / `INTERSECT` tree.
//!
//! # Cases
...
fn incorrect_cast_int_to_double_is_ok() {
    // It is consistent with Postgres. See test below.
    insta::assert_snapshot!(
...
fn order_by_is_rejected() {
    insta::assert_snapshot!(
        analyze_error("SELECT a FROM t1 ORDER BY a", &[]),
        @"failed to analyze AST: ORDER BY is not supported yet"
    );
}
...
fn group_by_is_rejected() {
    insta::assert_snapshot!(
        analyze_error("SELECT a FROM t1 GROUP BY a", &[]),
        @"failed to analyze AST: GROUP BY is not supported yet"
    );
}
...
fn having_is_rejected() {
    insta::assert_snapshot!(
        analyze_error("SELECT a FROM t1 HAVING true", &[]),
        @"failed to analyze AST: HAVING is not supported yet"
    );
}
