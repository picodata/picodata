...
use crate::frontend::sql::ast::{ParseTree, ParsingPairsMap};
...
use sql_executor::test_helpers::{expect_sql_to_ir_error, sql_to_optimized_ir};
...
fn front_sql_is_true() {
    ...
    let plan = sql_to_optimized_ir(input, vec![]);
    insta::assert_snapshot!(explain_logical(&plan).unwrap(), @"projection (true::bool = true::bool -> col_1)");

    let input = r#"explain (logical) select true is not true"#;
    let plan = sql_to_optimized_ir(input, vec![]);
    insta::assert_snapshot!(explain_logical(&plan).unwrap(), @"projection (not true::bool = true::bool -> col_1)");
}
...
fn front_sql_is_false() {
    ...
    let plan = sql_to_optimized_ir(input, vec![]);
    insta::assert_snapshot!(explain_logical(&plan).unwrap(), @"projection (true::bool = false::bool -> col_1)");

    let input = r#"explain (logical) select true is not false"#;
    let plan = sql_to_optimized_ir(input, vec![]);
    insta::assert_snapshot!(explain_logical(&plan).unwrap(), @"projection (not true::bool = false::bool -> col_1)");
}
...
fn front_sql_is_null_unknown() {
    ...
    let plan = sql_to_optimized_ir(input, vec![]);
    insta::assert_snapshot!(explain_logical(&plan).unwrap(), @"projection (true::bool is null -> col_1)");
    ...
    let plan = sql_to_optimized_ir(input, vec![]);
    insta::assert_snapshot!(explain_logical(&plan).unwrap(), @"projection (not true::bool is null -> col_1)");
...
fn front_sql_between_invalid() {
    ...
    for invalid_expr in invalid_between_expressions {
        let metadata = &RouterConfigurationMock::new();
        let plan = transform_into_plan(invalid_expr, &[], metadata);
...
fn front_sql_check_single_quotes_are_escaped() {
    ...
    insta::assert_snapshot!(explain_logical(&plan).unwrap(), @"projection (''::string -> col_1, '''::string -> col_2, 'left'right'::string -> col_3, ''center''::string -> col_4)");
}
...
fn front_order_by_with_param() {
    ...
    let plan = transform_into_plan(input, &params_types, metadata);
    let err = plan.unwrap_err();
...
fn get_positions(plan: &Plan, node_id: NodeId) -> Option<Positions> {
    let mut context = plan.context_mut();
    context
        .get_shard_columns_positions(node_id, plan)
        .unwrap()
        .copied()
}
...
fn track_shard_col_pos() {
    ...
    for node_id in dfs.traverse_into_iter(top) {
        ...
        match node {
            Relational::ScanRelation(_) | Relational::Selection(_) => {
                assert_eq!(
                    [Some(4_usize), None],
                    get_positions(&plan, node_id).unwrap()
                )
            }
    ...
    assert_eq!([Some(1_usize), None], get_positions(&plan, top).unwrap());
    ...
    let plan = sql_to_optimized_ir(input, vec![]);
    let top = plan.get_top().unwrap();
    assert_eq!([Some(0_usize), None], get_positions(&plan, top).unwrap());
...
fn front_sql_groupby() {
    ...
    projection (gr_expr_1::int -> identification_number, gr_expr_2::string -> product_code)
      group by (gr_expr_1::int, gr_expr_2::string)
        motion [policy: full, program: ReshardIfNeeded]
          projection (hash_testing.identification_number::int -> gr_expr_1, hash_testing.product_code::string -> gr_expr_2)
            group by (hash_testing.identification_number::int, hash_testing.product_code::string)
              scan hash_testing
    ");
}
...
fn front_sql_groupby_less_cols_in_proj() {
    ...
    projection (gr_expr_1::int -> identification_number)
      group by (gr_expr_1::int, gr_expr_2::bool)
        motion [policy: full, program: ReshardIfNeeded]
          projection (hash_testing.identification_number::int -> gr_expr_1, hash_testing.product_units::bool -> gr_expr_2)
            group by (hash_testing.identification_number::int, hash_testing.product_units::bool)
              scan hash_testing
    ");
}
          ...
          group by (gr_expr_1::int)
            motion [policy: full, program: ReshardIfNeeded]
              projection (hash_testing.identification_number::int -> gr_expr_1)
                group by (hash_testing.identification_number::int)
                  scan hash_testing
      projection (hash_testing.identification_number::int -> identification_number)
        scan hash_testing
    ");
...
fn front_sql_groupby_join_1() {
    ...
    projection (gr_expr_1::string -> product_code, gr_expr_2::bool -> product_units)
      group by (gr_expr_1::string, gr_expr_2::bool)
        motion [policy: full, program: ReshardIfNeeded]
          projection (t2.product_code::string -> gr_expr_1, t2.product_units::bool -> gr_expr_2)
            group by (t2.product_code::string, t2.product_units::bool)
      ...
      group by (gr_expr_1::int, gr_expr_2::int)
        motion [policy: full, program: ReshardIfNeeded]
          projection (unnamed_subquery.id::int -> gr_expr_1, unnamed_subquery_1.id::int -> gr_expr_2)
            group by (unnamed_subquery.id::int, unnamed_subquery_1.id::int)
              join on (true::bool)
                scan unnamed_subquery
    ...
    projection (group_concat(group_concat_1::string)::string -> col_1, group_concat(distinct gr_expr_1::string)::string -> col_2)
      motion [policy: full, program: ReshardIfNeeded]
        projection (test_space."FIRST_NAME"::string::string -> gr_expr_1, group_concat(test_space."FIRST_NAME"::string::string)::string -> group_concat_1)
          group by (test_space."FIRST_NAME"::string::string)
            scan test_space
    "#);
}
      ...
      group by (gr_expr_1::int)
        motion [policy: full, program: ReshardIfNeeded]
          projection (t.b::int -> gr_expr_1, t.a::int::int -> gr_expr_2, t.b::int::int -> gr_expr_3)
            group by (t.b::int, t.a::int::int, t.b::int::int)
              scan t
    ");
}
      ...
      group by (gr_expr_1::int)
        motion [policy: full, program: ReshardIfNeeded]
          projection (t.b::int -> gr_expr_1, (t.a::int + t.b::int + 3::int)::int -> gr_expr_2)
            group by (t.b::int, (t.a::int + t.b::int + 3::int)::int)
              scan t
    ");
}
...
fn front_sql_pg_style_params4() {
    ...
    let err = transform_into_plan(input, &params, metadata).unwrap_err();
...
fn front_sql_pg_style_params5() {
    ...
    let metadata = &RouterConfigurationMock::new();
    let err = transform_into_plan(input, &[], metadata).unwrap_err();
...
fn front_sql_pg_style_params6() {
    ...
    let metadata = &RouterConfigurationMock::new();
    let mut plan = transform_into_plan(input, &[], metadata).unwrap();
...
fn front_sql_pg_style_params7() {
    ...
    let metadata = &RouterConfigurationMock::new();
    let params = [DerivedType::new(Type::Integer)];
    let mut plan = transform_into_plan(input, &params, metadata).unwrap();
    let err = plan.bind_params(vec![], Options::default()).unwrap_err();
    assert_eq!(
        "invalid query: expected 1 values for parameters, got 0",
        err.to_string()
...
fn front_sql_tnt_style_params2() {
    ...
    insta::assert_snapshot!(explain_logical(&plan).unwrap(), @"projection ((1::int >= 1::int and 1::int <= 2::int) -> col_1)");
}
      ...
      group by (gr_expr_1::int, gr_expr_2::int)
        motion [policy: full, program: ReshardIfNeeded]
          projection (t.a::int + t.b::int -> gr_expr_1, t.c::int * t.d::int -> gr_expr_2, count((t.a::int * t.b::int)::int)::int -> count_2, sum((t.c::int * t.d::int)::int)::decimal -> sum_1)
            group by (t.a::int + t.b::int, t.c::int * t.d::int)
              scan t
    ");
}
      ...
      group by (gr_expr_1::int, gr_expr_2::int)
        motion [policy: full, program: ReshardIfNeeded]
          projection (t.a::int + t.b::int -> gr_expr_1, t.a::int -> gr_expr_2)
            group by (t.a::int + t.b::int, t.a::int)
              scan t
    ");
}
      ...
      group by (gr_expr_1::int, gr_expr_2::int)
        motion [policy: full, program: ReshardIfNeeded]
          projection (t.a::int + ROW($0) -> gr_expr_1, t.b::int -> gr_expr_2)
            group by (t.a::int + ROW($0), t.b::int)
              scan t
    subquery $0:
      scan
        projection (1::int -> col_1)
      ...
      group by (gr_expr_1::int)
        motion [policy: full, program: ReshardIfNeeded]
          projection (t.a::int + ROW($1) -> gr_expr_1, (t.a::int + ROW($0))::int -> gr_expr_2)
            group by (t.a::int + ROW($1), (t.a::int + ROW($0))::int)
              scan t
    subquery $0:
      scan
        projection (1::int -> col_1)
...
fn front_sql_having_with_sq() {
    ...
    projection (gr_expr_1::int -> "sysFrom", sum(distinct gr_expr_2::decimal)::decimal -> sum, count(distinct gr_expr_2::int)::int -> count)
      having (ROW($0) > count(distinct gr_expr_2::int)::int)
        group by (gr_expr_1::int)
          motion [policy: full, program: ReshardIfNeeded]
            projection (test_space."sysFrom"::int -> gr_expr_1, test_space.id::int::int -> gr_expr_2)
              group by (test_space."sysFrom"::int, test_space.id::int::int)
                scan test_space
    subquery $0:
      motion [policy: full, program: ReshardIfNeeded]
        scan
...
fn front_sql_having_with_sq_segment_motion() {
    ...
    projection (gr_expr_1::int -> "sysFrom", gr_expr_2::int -> sys_op, sum(distinct gr_expr_3::decimal)::decimal -> sum, count(distinct gr_expr_3::int)::int -> count)
      having (ROW(gr_expr_1::int, gr_expr_2::int) in ROW($0, $0))
        group by (gr_expr_1::int, gr_expr_2::int)
          motion [policy: full, program: ReshardIfNeeded]
            projection (test_space."sysFrom"::int -> gr_expr_1, test_space.sys_op::int -> gr_expr_2, test_space.id::int::int -> gr_expr_3)
              group by (test_space."sysFrom"::int, test_space.sys_op::int, test_space.id::int::int)
                scan test_space
    subquery $0:
      motion [policy: full, program: ReshardIfNeeded]
        scan
      ...
      group by (gr_expr_1::int, gr_expr_2::int)
        motion [policy: full, program: ReshardIfNeeded]
          projection (t.a::int -> gr_expr_1, t.a::int + t.b::int -> gr_expr_2)
            group by (t.a::int, t.a::int + t.b::int)
              scan t
    ");
}
...
fn front_sql_select_distinct_asterisk() {
    ...
    projection (gr_expr_1::int -> a, gr_expr_2::int -> b, gr_expr_3::int -> c, gr_expr_4::int -> d)
      group by (gr_expr_1::int, gr_expr_2::int, gr_expr_3::int, gr_expr_4::int)
        motion [policy: full, program: ReshardIfNeeded]
...
fn front_sql_current_date() {
  ...
  selection (to_date('2010/10/10'::string, '%Y/%d/%m'::string)::datetime < '{today}'::datetime)
    ...
    assert_eq!(expected_explain, explain_logical(&plan).unwrap());
}
...
fn front_mock_partition_by() {
    ...
    for query in queries_to_check {
        let err = transform_into_plan(query, &[], metadata).unwrap_err();
        assert!(err
            .to_string()
      ...
      group by (gr_expr_1::int)
        motion [policy: full, program: ReshardIfNeeded]
          projection (test_space.id::int + ROW($0) -> gr_expr_1, count(*)::int -> count_1)
            group by (test_space.id::int + ROW($0))
              scan test_space
    subquery $0:
      scan
        motion [policy: full, program: ReshardIfNeeded]
...
fn front_select_without_scan() {
    ...
    insta::assert_snapshot!(explain_logical(&plan).unwrap(), @"projection (1::int -> col_1)");
}
...
fn front_sql_whitespaces_are_not_ignored() {
    ...
    for query in correct_queries {
        let res = ParseTree::parse(crate::frontend::sql::ast::Rule::Command, query);
        assert!(res.is_ok());
    }
    ...
    for query in correct_queries {
        ...
        for wp_idx in whitespaces {
            ...
            let res = ParseTree::parse(crate::frontend::sql::ast::Rule::Command, &fixed);
            assert!(res.is_err())
        }
    }
}
...
fn front_sql_check_in_statement() {
    ...
    for input in correct_statements {
        let _ = transform_into_plan(input, &[], metadata).unwrap();
    }
...
mod multi_queries {
    ...
    fn parse(query: &str) -> Result<(), crate::errors::SbroadError> {
        let mut map1 = ParsingPairsMap::new();
        let mut map2 = HashMap::new();
        let mut map3 = HashMap::new();
        let mut standard_parse = crate::frontend::sql::ast::AbstractSyntaxTree::empty();
