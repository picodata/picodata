...
fn to_test_motion(plan: &Plan, node_id: NodeId) -> Policy {
    ...
    match node {
        Relational::Motion(Motion { policy, .. }) => match policy {
            ...
            | MotionPolicy::LocalSegment(MotionKey { targets }) => {
                ...
                let aliases = plan.get_relational_aliases(node_id).unwrap();
                for t in targets {
                    match t {
...
fn check_join_motions(
    ...
    let join_id = dfs
        .traverse_into_iter(plan.get_top().unwrap())
        .find(|id| -> bool {
    ...
    let (left_actual, right_actual) = (to_test_motion(&plan, *left), to_test_motion(&plan, *right));
