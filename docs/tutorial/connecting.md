...
Подключение по протоколу PostgreSQL доступно для пользователей,
использующих методы аутентификации `md5` и `ldap`. В случае
с `ldap` потребуется [дополнительная настройка](../admin/ldap.md).

### Настройка и подключение {: #setup }

Протокол PostgreSQL включен по умолчанию и позволяет инстансу Picodata
принимать подключения в качестве сервера PostgreSQL.

Адрес подключения: `127.0.0.1:4327`

Для изменения адреса подключения запустите инстанс Picodata с параметром
[`picodata run --pg-listen`], указав новый адрес в формате
`[host][:port]`:
...
```shell
psql postgres://alice:T0psecret@127.0.0.1:5432
```
