...
!!! info "Пароль администратора должен удовлетворять [требованиям к паролю]."

[требованиям к паролю]: access_control.md#allowed_passwords

Кроме того, чтобы задать пароль администратора, используйте переменную
окружения `PICODATA_ADMIN_PASSWORD`. Picodata установит значение
переменной окружения в качестве пароля администратора при *первом* запуске
инстанса — при повторных запусках данная переменная игнорируется. В случае
успешной установки пароля в *stdout* будет присутствовать следующая запись:

```shell
main/103/interactive I> Password for user=admin has been set successfully
```
