...
## Минимальный вариант кластера {: #minimal_cluster }
...
рабочую директорию (параметр `--data-dir`), в которой будут размещаться
[рабочие файлы инстанса](#instance_runtime_files). Также нужно указать
адрес и порт для приема соединений (параметр `--listen`) в формате
`<HOST>:<PORT>`. Фактор репликации по умолчанию равен 1 — каждый инстанс
образует отдельный репликасет. Если для `--listen` указать только порт,
то будет использован IP-адрес по умолчанию (127.0.0.1):
...
## Подключение к кластеру {: #accessing_console }
...
`picodata connect` (см. [подробнее](../reference/cli.md#connect_command)).
...
### Удаление инстанса с помощью консольной команды {: #expulsion_command }
...
### Удаление инстанса из консоли Picodata с помощью Lua API {: #expulsion_lua_api }
