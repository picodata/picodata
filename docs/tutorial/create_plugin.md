...
Обратите внимание на параметра `--lib` — мы будем собирать библиотеку вида
...
Для создания нового проекта плагина из шаблона предусмотрены две команды:

```shell title="Создание плагина в указанной директории"
cargo pike plugin new name_of_new_plugin
```

```shell title="Создание плагина в текущей директории"
cargo pike plugin init
```

В обоих случаях автоматически инициализируется проект git. Для
отключения этого поведения используйте параметр `--without-git`.
...
Создадим шаблон будущего плагина:
...
Если необходимо оформить проект без Git-репозитория, добавьте параметр
`--without-git`:
...
Для сборки плагина без упаковки используйте следующую команду:

```shell
cargo pike plugin build
```

Команда `plugin build` по умолчанию собирает debug-версию плагина.
