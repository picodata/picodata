...
Для более удобной разработки плагина воспользуйтесь Pike — созданной в Picodata
консольной утилитой, упрощающей многие операции с плагинами. Pike позволяет:

- быстро создавать шаблоны плагинов
- автоматизировать создание архива с файлами плагина для его последующего деплоя
- удобно запускать тестовый кластер Picodata для тестирования плагина

См. также:

- [Описание Pike](https://github.com/picodata/pike)


### Создание плагина {: #create_plugin_with_pike}

Рассмотрим процесс разработки плагина с помощью Pike. Для создания шаблона
будущего плагина в отдельной поддиректории введите следующую команду:

```shell
cargo pike plugin new super_plugin
```

После этого в `super_plugin` появится следующая структура файлов:
...
Существует второй способ создания шаблона: если вы уже находитесь в
ранее созданной директории, в которой предполагается разработка плагина.

Следующая команда инициализирует проекта плагина в виде Git-репозитория
...
Если необходимо оформить проект без Git-репозитория, добавьте флаг
`--without-git`:

```shell
cargo pike plugin init --without-git
```

В директории `src` находятся файлы собирающегося и устанавливающегося
плагина. Проект плагина сразу готов для сборки с помощью Cargo. В `tests`
предоставляется набор встроенных тестов для создания, сборки и проверки
работы плагина в кластере.

### Сборка и упаковка плагина {: #build_plugin_with_pike}

Для сборки плагина в архив `.tar.gz` для доставки на
сервера посредствам роли Ansible или Helm-чарта используйте следующую
команду:
...
Архив появится в директории `target`.
...
кластера Picodata. Параметры кластера и входящих в него [инстансов] определяются в
файле `topology.toml`. По умолчанию будет запущен кластер из 2 инстансов,
которые объединятся в 1 [репликасет].

[инстансов]: ../overview/glossary.md#instance
[репликасет]: ../overview/glossary.md#replicaset
