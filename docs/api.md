...
Функция принимает в качестве параметра число секунд перед применением
отмены. Возвращает номер соответствующей операции `DdlAbort` в
raft-журнале, либо сообщение об отсутствие ожидающиз операций.
