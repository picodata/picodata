...
| Функция                       | Описание                          |
|-------------------------------|-----------------------------------|
| [pico.LUA_API_VERSION](#picolua_api_version) | Версия Lua API в Picodata.                  |
...
| [pico.create_role()](#picocreate_role) | Создание роли в Picodata.
| [pico.create_space()](#picocreate_space) | Создание спейса в Picodata.
| [pico.create_user()](#picocreate_user) | Создание пользователя в Picodata.
| [pico.drop_role()](#picodrop_role) | Удаление роли в Picodata.
| [pico.drop_space()](#picodrop_space) | Удаление спейса в Picodata.
...
### pico.create_role
Создает новую роль на каждом инстансе кластера.
...
### pico.create_space
Создает спейс (пространство для хранения данных) в Picodata
...
(_number_) или (_nil_, _string_) в случае ошибки
...
```
### pico.create_user
Создает нового пользователя на каждом инстансе кластера.
...
### pico.drop_role
Удаляет роль и связанные с ней сущности с каждого инстанса кластера. Например, будут удалены спейсы, владельцем которых была эта роль.
...
### pico.drop_space
Удаляет спейс (пространство для хранения данных) в Picodata на всех инстансах кластера
...
### pico.drop_user
Удаляет пользователя и связанные с ним сущности с каждого инстанса кластера. Например, будут удалены спейсы, владельцем которых был этот пользователь.
...
(_number_) с номером raft-индекса или (_nil_, _error_ (ошибка в виде Lua-объекта)) в случае ошибки
  ...
  если пользователь был удален локально. При повторном вызове она будет
  возвращать сообщение "пользователь не найден", несмотря на то, что
  удаление в дальнейшем может быть отменено или подтверждено. 
