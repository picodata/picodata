...
параметром `password_min_length` в системном спейсе `_pico_property` (по
...
пользователя, определяемые параметром `password_min_length` в системном
спейсе `_pico_property` (по умолчанию требуется пароль не короче 8
символов). 
