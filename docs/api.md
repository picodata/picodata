...
конфликтов, то функция добавляет новую запись в raft-журнал и
возвращает ее индекс (пока еще не зафиксированный в кластере).
...
генерирует для raft-журнала запись, которая при применении изменяет пароль пользователя.
Для ожидания локального создания используется таймаут. Результатом
успешного выполнения функции является индекс соответствующей записи в
raft-журнале. Если новый пароль не отличается от старого, то запрос игнорируется.
...
Создает спейс (пространство для хранения данных). Функция завершится,
как только спейс будет создан глобально и доступен с текущего инстанса.
...
инстансами. Если такой спейс уже существует, то запрос игнорируется.
...
Для чтения из глобального спейса используется box-API Tarantool:
...
превышает таймаут, функция возвращает ошибку.  Если спейса не
...
Пример удаления шардированного спейса:
...
Пример параметризированной вставки в шардированный спейс:
...
Пример чтения из шардированного спейса:
...
### table CasRange <a name="casrange"></a>
...
```
### table DqlResult <a name="dql_table"></a>
Lua-таблица, содержащая данные чтения из шардированного спейса.

Поля:

- `metadata` (_table_), массив столбцов таблицы (спейса) в формате `{{name = string, type = string}, ...}`.
- `rows` (_table_), результат выполнения читающего запроса в формате `{row, ...}`.

### table DmlResult <a name="dml_table"></a>
Lua-таблица, содержащая количество измененных строк при модификации шардированного спейса.
...
Lua-таблица, описывающая поле в составе спейса (см. [pico.create_space](#picocreate_space)).
