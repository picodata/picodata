...
- Data Modification Language (`DML`): команды [`INSERT`](#insert) для
  добавления данных в спейсы и [`DELETE`](#delete) для удаления данных;
...
## Использование UNION ALL <a name="union_all"></a>
...
## Использование опций неблокирующего SQL <a name="tuple_limit"></a>
При работе с распределенными SQL-запросами следует принимать в расчет
следующие факторы:

- в зависимости от топологии кластера, SQL-запрос может вызывать
  значительные задержки и блокировать транзакционный поток, что будет
  мешать выполнению других запросов;
...
## Запрос DELETE <a name="delete"></a>
Команда `DELETE` используется для удаления данных из спейсов.
