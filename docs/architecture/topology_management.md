...
- raft-лидер, получив этот запрос, выполняет ряд транзакций в глобальные
  таблицы:
    - если версия инстанса не соответствует версии лидера кластера, запрос отклоняется
    ...
    - происходит сравнение между глобальной версии кластера (хранится в
      системной таблице `_pico_property`) и версией инстанса, который
      отправил запрос на присоединение (поле `picodata_version` в
      системной таблице `_pico_instance`). Если версия инстанса Picodata
      несовместима, запрос отклоняется (допускается различие на 1
      минорную версию, то есть, если кластер с версиями 25.1, то
      добавиться могут только инстансы версий 25.1 и 25.2)
