...
Используйте следующие команды ддля установки необходимых зависимостей для сборки Picodata из исходного кода. Обратите внимание, что потребует установить актуальные версии Rust и Cargo, используя официальные инструкции с сайта [rustup.rs](rustup.rs):
```bash
...
-->
Подробнее о запуске Picodata и работе с кластером см. разделе [Инструкция по развертыванию](../deploy). Параметры запуска из командной строки описаны в разделе [Описание параметров запуска](../cli)
