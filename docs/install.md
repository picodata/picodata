...
Каждый инстанс Picodata — это отдельный процесс в ОС. Для его диагностики удобно воспользоваться командами из встроенной консоли Tarantool, которая автоматически открывается после запуска инстанса (`picodata run ...`). Для просмотра состояния отдельного инстанса после его запуска введите следующую команду:
```
box.space.raft_log:fselect()
```
Для диагностики всей Raft-группы (например, для оценки количества инстансов в кластере) команда будет немного другой:
