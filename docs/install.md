...
репозитории с готовыми пакетами для RHEL/CentOS 7-8, Ubuntu 20.04, 22.04
и Alt Linux p10. Внутри пакетов находится статически слинкованная версия
исполняемого файла `picodata`. Более подробная информация об установке
приведена на сайте
...
инструкции с сайта [rustup.rs](rustup.rs):
```bash
