...
к одному региону и, следовательно, не попадут в один репликасет. <!-- исключения описаны ниже -->
...
Удаление - это принятие кластером решения, что некий инстанс больше не является участником кластера. После удаления кластер больше не требует присутствие инстанса в кворуме, а сам инстанс завершается. Если удаляется текущий лидер - принудительно запускается выбор нового лидера.
...
- в `192.168.100.123:3301` будет отправлена команда `expel` с `instance-id = "i3"` и дефолтным `cluster-id`. Инстанс на `192.168.100.123:3301` найдёт лидера и отправит ему команду `expel`. Лидер отметит что указанный инстанс удалён, остальные инстансы получат эту информацию через raft. Если удаляемый инстанс включен - он завершится, если выключен - примет информацию о своём удалении при включении и завершится. При последующем запуске удалённый инстанс будет сразу завершаться.

### Удаление инстанса из консоли Picodata с помощью Lua API

В консоли запущенного инстанса наберите
...
Будет удалён инстанс `i3`. Сам инстанс i3 будет завершён. Если вы находитесь в консоли удаляемого инстанса - процесс завершится, и консоль будет закрыта.
