...
`picodata run --help`. С точки зрения внутренней архитектуры, [_кластер_](glossary.md#cluster)
  корректно называть [_raft-группой_](glossary.md#raft-group) — в дальнейшем при мониторинге и
...
См. [отдельный подраздел](../api) с описанием Lua API в Picodata. Дополнительно о внутренней архитектуре кластера Picodata можно узнать в разделе [Cхема инициализации кластера](../clustering).
