...
1. Запустим кластер из двух экземпляров
    ...
    ```
1. Создадим шардированную таблицу (space)
    
    Ключевым моментом является создание индекса `bucket_id` в space (его наличие помечает space как шардированный). 
    ```lua
...
1. Заполним шардированный space (все 3000 бакетов) тестовыми данными
    ```lua
