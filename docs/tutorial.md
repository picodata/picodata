...
набором параметров для каждого инстанса. Полный перечень возможных
параметров запуска и их описание содержатся в подразделе [Описание
параметров запуска](../cli), а также в выводе команды `picodata run
--help`. Для данного примера допустим, что в локальном кластере
...
## Мониторинг состояния кластера
Для мониторинга состояния кластера удобно использовать небольшой
Bash-скрипт следующего содержания:
```bash
#!/bin/bash
...
_SCRIPT_
```
Для постоянного мониторинга удобно сохранить данный код в виде
Shell-скрипта (например `monitor.sh`) и выполнять его в отдельном
терминале в следующем виде:
```bash
watch ./monitor.sh
```
