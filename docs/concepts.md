...
Данный раздел содержит описание концепций и терминов, используемых в Picodata и необходимых для понимания работы распределенной СУБД.
...
### Состояния узлов в Raft-группе {: #node-states }
...
### Репликация raft-журнала {: #log-replication }
...
**Группа инстансов** является более крупным образованием, чем репликасет. См пояснительную [картинку](concepts.md#cluster_group).
...
- [RFC — Storage schema — Raft snapshot](https://docs.google.com/document/d/1MEpGnpKKj6WezLKytvvonZzbpy1tlWtAK5ccxaeAOrE/edit#heading=h.687c3wywf9ub){:target="_blank"}
- [Tarantool — Persistence](https://www.tarantool.io/en/doc/latest/concepts/data_model/persistence/){:target="_blank"}


## Процессы и алгоритмы {: #processes-and-algorithms }
### Компактизация raft-журнала (raft log compaction) {: #raft-log-compaction }
...
Уровень изоляции транзакций является одним из компонентов ACID (_atomicity, consistency, isolation, durability_), который представляет собой набор требований к СУБД согласно стандарту [ANSI/ISO SQL](https://en.wikipedia.org/wiki/ISO/IEC_9075){:target="_blank"}. Уровень изоляции транзакций определяет степень строгости, которую СУБД применяет к возможной несогласованности данных при исполнении нескольких транзакций одновременно. Различают следующие уровни (по мере повышения строгости и усиления блокировок):
...
### Read phenomena (проблемы чтения данных) {: #read-phenomena }
...
---
[Исходный код страницы](https://git.picodata.io/picodata/picodata/docs/-/blob/main/docs/concepts.md)
