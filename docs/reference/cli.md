...
--unix`](#connect-options). В отличие от подключения к адресу,
указанному в параметре `--listen`, данный способ использует обычный текстовый протокол и привилегии администратора. Аналогичная переменная окружения:
`PICODATA_CONSOLE_SOCK`.

#### audit {: #audit}
`--audit <file>`<br>
Файл журнала. Данный параметр включает запись событий инстанса в указанный файл журнала. В качестве файла может быть указан:
...
- `--unix`, подключение к файлу сокета. С этим параметром
  обязательный аргумент <ADDRESS> интерпретируется как путь до сокета
