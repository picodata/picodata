...
## Регистрируемая информация о событиях {: #collected-info }
...
- `time` – дата и время (временная метка)
- `title` – наименование события безопасности (например, `create_user`)
- `level` – важность события (`высокая` / `средняя` / `низкая`)
- `user` – субъект доступа, т.е. имя пользователя, выполнившего операцию
  (если применимо)
