...
при исполнении скомпилированного запроса из опкодов в VDBE-машине. Под
"командой" в данном случае понимается прохождение исполнения через
опкод. Число команд прямо пропорционально числу исполненных опкодов из
плана запроса.

Ограничение `SQL_VDBE_MAX_STEPS` позволяет предотвратить выполнение
запроса в тех случаях, если запрашиваемая таблица слишком велика и
сигнализировать пользователю о необходимости переделать запрос (например
отказаться от полного сканирования (`SELECT * ...`) такой таблицы).
...
других действий нужно проделать запросу — тем выше будет это значение.
