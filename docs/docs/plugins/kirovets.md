...
??? note "Основные термины"
      ...
      **Мутатор (Mutator)** — это пользовательская Lua-функция, выполняемая непосредственно перед фильтрацией,
...
- `config.yml` содержит описание всех компонентов системы: таблиц, импортеров, системных настроек, задач и API.
...
- `files` (обязательный) — описывает источник Parquet-файлов:
  - `type` (обязательный) — тип источника:
    - `dir` — загрузка всех файлов из указанной директории;
    - `list` — загрузка конкретного списка файлов.
  - `dir` — путь к директории с Parquet-файлами. Указывается при `type: dir`.
  - `pattern` — необязательный шаблон имени файла (с подстановочными знаками `*` и `?`) для отбора файлов внутри директории.
    Указывается при `type: dir`. Если ни один файл в директории не соответствует шаблону, импорт завершится ошибкой.
  - `list` — список путей к файлам. Указывается при `type: list`.
- `prune` — управляет очисткой записей, отсутствующих в загруженном снимке (по умолчанию `true`):
  - `true` — по завершении импорта удаляются записи, не вошедшие в загруженный снимок;
  - `false` — очистка не выполняется.

!!! warning "Использование `prune` совместно с фильтрами"
    Если на таблице настроен [фильтр](#filter_def), основанный на сравнении старой и новой
    записей, необходимо установить `prune: false`. Типичный пример — фильтр сравнивает временную
    метку (или версию) записи и отклоняет операцию, если новые данные не новее уже сохранённых.
    Если в снимок попадает устаревшая версия записи по сравнению с той, что уже находится в
    таблице, фильтр её отклоняет, но при включённом параметре `prune: true` уже существующая
    актуальная строка будет ошибочно удалена по завершении загрузки, поскольку система посчитает
    её отсутствующей в снимке.
...
# Загрузка всех файлов из директории, соответствующих шаблону
sources:
  parquet:
    files:
      type: "dir"
...
Используется для перемещения данных из промежуточной таблицы в целевую.
Предназначен для случая, когда данные загружаются напрямую в промежуточную таблицу с помощью драйвера.

Доступные параметры:

- `table` (обязательный) — произвольная метка источника. Значение поля ни на что не влияет
  и не обязано совпадать с именем промежуточной или целевой таблицы: имя промежуточной таблицы
  формируется автоматически, см. [Internal](#internal)
...
#### Метаинформация источника {: #meta}

Таблица `meta` доступна во всех пользовательских функциях обработки данных как поле `new.meta` нового объекта.
Она описывает контекст операции: источник данных, тип изменения и временные метки.

##### Структура таблицы `meta` {: #meta_table_structure }

```lua
{
    source     = string,         -- "ogg" | "dataflot" | "debezium-kafka" | "parquet" | "internal"
    operation  = string,         -- "create" | "update" | "delete"
    changed_at = datetime | nil, -- момент изменения на стороне источника
    lag        = number   | nil, -- отставание в секундах (now - changed_at)
    deleted    = boolean  | nil, -- признак удаления (только Debezium-Kafka)
}
```

##### Соответствие полей источника целевым полям меты {: #meta_field_mapping }

###### OGG {: #meta_ogg }

Тип операции выводится из данных строки: если все старые значения `nil` — это `"create"`,
все новые `nil` — `"delete"`, иначе — `"update"`. Временная метка берётся
из опционального заголовка строки вида `"mb"|"<timestamp>"|"me"|`.

| Поле `meta`  | Исходное поле / правило                               |
|:-------------|:------------------------------------------------------|
| `source`     | всегда `"ogg"`                                        |
| `operation`  | выводится из данных (см. выше)                        |
| `changed_at` | timestamp из заголовка; `nil` если заголовок отсутствует |
| `lag`        | `now - changed_at`; `nil` если `changed_at` отсутствует |
| `deleted`    | не передаётся                                         |

###### Dataflot {: #meta_dataflot }

Каждая строка файла начинается с заголовка:
`<op_time>;<op_type>;<lsn>;<commit_time>;<tx_id>;<seq>;`

| Поле `meta`  | Исходное поле / правило                                        |
|:-------------|:---------------------------------------------------------------|
| `source`     | всегда `"dataflot"`                                            |
| `operation`  | `op_type`: `I` → `"create"`, `U` → `"update"`, `D` → `"delete"` |
| `changed_at` | `commit_time`                                                  |
| `lag`        | `now - commit_time`                                            |
| `deleted`    | не передаётся                                                  |

###### Debezium-Kafka {: #meta_debezium_kafka }

Поля читаются из служебных атрибутов Kafka-сообщения (префикс `__`).

| Поле `meta`  | Исходное поле / правило                                                        |
|:-------------|:-------------------------------------------------------------------------------|
| `source`     | всегда `"debezium-kafka"`                                                      |
| `operation`  | `__op`: `r`/`c` → `"create"`, `u` → `"update"`, `d` → `"delete"`             |
| `changed_at` | `__source_ts_ms` / 1000; `nil` если поле отсутствует                          |
| `lag`        | `now - changed_at`; `nil` если `changed_at` отсутствует                       |
| `deleted`    | `__deleted`: `"true"` → `true`, `"false"` → `false`; `nil` если поля нет      |

###### Parquet {: #meta_parquet }

Parquet не содержит информации о типе операции или времени изменения.

| Поле `meta`  | Значение            |
|:-------------|:--------------------|
| `source`     | всегда `"parquet"`  |
| `operation`  | всегда `"create"`   |
| `changed_at` | не передаётся       |
| `lag`        | не передаётся       |
| `deleted`    | не передаётся       |

###### Internal {: #meta_internal }

Лендинговая таблица не содержит информации о типе операции или времени изменения: каждая
перенесённая запись обрабатывается как создание новой.

| Поле `meta`  | Значение            |
|:-------------|:--------------------|
| `source`     | всегда `"internal"` |
| `operation`  | всегда `"create"`   |
| `changed_at` | не передаётся       |
| `lag`        | не передаётся       |
| `deleted`    | не передаётся       |
...
Логика фильтрации должна учитывать наличие трех видов операций:

- **Операция `update`**
 Только в этом случае оба объекта (`old` и `new`) представлены непустыми таблицами, поскольку обновление предполагает наличие как исходных данных, так и данных после изменения.
- **Операция `create`**
 В случае операции `create` старый объект (`old`) представлен пустой таблицей, так как данные до создания записи отсутствуют.
- **Операция `delete`**
 В случае операции `delete` новый объект (`new`) представлен пустой таблицей, так как после удаления записи данных для объекта больше не существует.

Тип операции определяется через поле `new.meta.operation`.

Пример пользовательской функции, учитывающей все описанные случаи:

```lua
local function filter(old, new)
    -- Пропускаем все операции, кроме update
    if new.meta.operation ~= 'update' then
        return true
    end
...
Пользовательский триггер — это Lua-функция, позволяющая
...
```lua
local function trigger1(old, new)
    -- Логика триггера
    return true
end

local function trigger2(old, new)
    -- Логика триггера
    return true
end
...
```lua
local function trigger(old, new)
    -- Логика фильтрации
end
```
...
Логика триггера должна учитывать наличие трех видов операций:
...
```lua
local function update_destination(old, new)
    if new.meta.operation == 'delete' then --- игнорируем операции delete
        return true
    end
...
Мутаторы необходимо указать в конфигурации системы. Для каждой таблицы можно определить свой мутатор,
который задается в секции `schema.<имя_таблицы>` с помощью параметра `mutator`.
...
Функция-мутатор **должна** возвращать измененный объект `new`.
...
- `internal` — перемещает данные из предварительно заполненной промежуточной таблицы в целевую
...
- `journal` — включает [журнальный режим](#journal_mode) для этого источника
  (по умолчанию `false`)
- `journal_ttl` — время хранения записей в журнале при включённом `journal`
  (по умолчанию `7d`). Задаётся в виде человекочитаемой строки длительности,
  например: `2h`, `30m`, `7d`
...
- `type` (обязательный) — тип импортера данных. В данном случае `parquet`

#### KafkaDebezium {: #kafka_debezium }

Доступные параметры для настройки импортёра:

- `type` (обязательный) — тип импортера, необходимо установить значение `kafkadebezium`.
...
- `kafka_batch_size` — количество сообщений, которое будет считываться из Kafka за один раз (значение по умолчанию: `100`).
- `step_hard_duration` — максимальное время обработки одного батча сообщений из Kafka (значение по умолчанию: `30s`).
  Если за этот промежуток батч не был обработан, текущая итерация прерывается.
  Задаётся в виде человекочитаемой строки длительности, например: `30s`, `1m`, `2m30s`.
- `step_delay` — задержка после завершения каждой итерации пайплайна перед началом следующей (значение по умолчанию: `100ms`).
  Позволяет снизить нагрузку на брокер в случае, если очередь Kafka пуста или данные поступают медленно.
  Задаётся в виде человекочитаемой строки длительности, например: `100ms`, `500ms`, `1s`.
- `kafka_retry_attempts` — количество повторных попыток чтения сообщений из Kafka при временных ошибках (значение по умолчанию: `3`). Когда все попытки обработки для конкретного топика завершаются неудачей, обработка этого топика прекращается без автоматического возобновления.
...
- `type` (обязательный) — тип импортера, необходимо установить значение dataflot.
- `file_dir` (обязательный) — путь к корневой директории, где хранятся данные, сгрупированные по циклу применения.
...
Импортер загружает данные напрямую через внешние коннекторы, минуя промежуточные файлы. Это ключевое отличие от файловых импортеров (например, [Parquet](#parquet)).

Процесс импорта включает два этапа:
1. Загрузка данных в промежуточную таблицу с помощью внешнего коннектора.
2. Перемещение данных в целевую таблицу с применением пользовательских функций:
   * [Мутатор](#custom_mutators)
   * [Фильтр](#custom_filters)
   * [Триггеры](#custom_triggers)

Импортер загружает в целевую таблицу полный снимок данных из промежуточной таблицы. Другими словами, он работает до полного опустошения промежуточной таблицы. Это значит, что к моменту запуска импортера снимок должен быть полностью загружен — иначе часть данных будет потеряна.

Управлять запуском можно двумя способами:
* Настроить [задачу](#triggers_tasks) с автоматическим расписанием.
* Настроить задачу с отключённым автозапуском (`enabled: false`) и принудительно выполнять её через
  [API управления задачами](#tasks_api_exec), например, сразу после завершения выгрузки через внешний коннектор.

!!! note
    Internal — пакетный импортер, поэтому запуск через [API управления импортерами](#importers_api) для него
    недоступен: эти эндпоинты предназначены только для потоковых импортеров.

Для заполнения промежуточной таблицы доступны следующие коннекторы:

* [JDBC](https://docs.picodata.io/picodata/stable/dev/connectors/jdbc/)
* [Go](https://docs.picodata.io/picodata/stable/dev/connectors/go/)
* [Rust](https://docs.picodata.io/picodata/stable/dev/connectors/rust/)
* [Picopyn](https://docs.picodata.io/picodata/stable/dev/connectors/picopyn/)

Промежуточная таблица создается автоматически при применении конфигурации, если определен соответствующий источник данных. Имя таблицы формируется по шаблону `landing_<target_table>`, а схема данных полностью совпадает со схемой целевой таблицы.

Доступные параметры настройки импортера:

* `type` (обязательный) — тип импортера. Должен быть указан как `internal`.
* `max_batch_size` — размер пакета записей (по умолчанию `1000`), который за один раз вычитывается из промежуточной таблицы и отправляется в хранилище.
* `tables` — параметры отдельных таблиц. Ключ объекта — имя таблицы, значение — объект с параметрами:
  * `prune` — управляет очисткой записей, отсутствующих в загруженном снимке (по умолчанию `true`):
    * `true` — по завершении импорта записи, не вошедшие в снимок, удаляются;
    * `false` — очистка не выполняется.
...
#### Журнальный режим (`journal`) {: #journal_mode }

По умолчанию каждая операция, пришедшая от потокового импортера (OGG,
KafkaDebezium, Dataflot), применяется к целевой таблице напрямую
с учетом пользовательских функций ([мутаторов](#custom_mutators), [фильтров](#custom_filters) и [триггеров](#custom_triggers)).

Журнальный режим меняет эту схему: вместо применения изменений к целевой таблице
каждая операция сохраняется как отдельная запись в служебной журнальной
таблице, а целевая таблица этим источником не изменяется.

Этот режим может быть полезен в следующем случае: фактическое состояние
целевой таблицы формируется другим источником (например, периодической
загрузкой слепка данных через Parquet), а поток изменений нужен только для
того, чтобы реагировать на каждое отдельное событие (например,
инкрементально обновлять агрегаты в другой таблице).

Включается для потокового источника параметром `journal: true` (доступен
для [OGG](#ogg), [KafkaDebezium](#kafka_debezium) и [Dataflot](#datflot)).

**Особенности работы импортера в журнальном режиме:**

- [Мутатор](#custom_mutators) и [фильтр](#custom_filters), заданные для таблицы,
  в этом режиме игнорируются — в журнал попадают исходные значения операции,
  без изменений.
- [Триггеры](#custom_triggers) выполняются как обычно, для каждой операции отдельно.
- Операция дополнительно сохраняется в журнальной таблице
  `journal_<имя_целевой_таблицы>` — она создаётся автоматически, если хотя
  бы один источник таблицы использует `journal: true`.
- Журнальная таблица доступна для чтения как обычная таблица, например,
  через [SQL-витрину](#sql_handler).

**Структура журнальной таблицы:**

Таблица состоит из полей целевой таблицы (все, кроме исходного первичного ключа,
становятся nullable — при `update` в записи заполнены только изменившиеся
поля) и служебных полей:

| Поле       | Тип       | Описание                                             |
|:-----------|:----------|:------------------------------------------------------|
| `_op_id`   | unsigned  | Порядковый номер операции, часть первичного ключа вместе с исходным PK |
| `_op_type` | string    | Тип операции: `create`, `update` или `delete`         |
| `_op_time` | integer   | `changed_at` из метаданных операции (если источник его передаёт) |
| `_source`  | string    | Источник операции (`ogg`, `dataflot` или `debezium-kafka`) |
| `_lag`     | integer   | `lag` из метаданных операции (если доступен)           |

**Хранение и очистка:**

Записи в журнальной таблице не хранятся вечно: фоновый процесс периодически
удаляет записи старше `journal_ttl` (по умолчанию `7d`). Если на одну и ту
же целевую таблицу настроено несколько источников с `journal: true` и
разными `journal_ttl`, для очистки используется наименьшее из указанных
значений.

**Пример:** таблица `card_state` получает актуальное состояние из
`parquet`-снимка, а поток изменений из OGG используется только для того,
чтобы инкрементально обновлять агрегат в таблице `client_balance`:
  ...
  card_state:
    ...
    sources:
      parquet:
        files:
          type: dir
          dir: dev/example_data/parquets/card_state
      ogg:
        table: CARDS
    triggers:
      - triggers.apply_delta
...
return {
    apply_delta = function(old, new)
        if new.meta.operation == 'delete' then
            return true
        end

        local rv, err = sql.execute('select * from client_balance where client_id=?', { new.client_id });
        if err then
            return nil, err
        end
        ...
        if not row then
            return sql.execute(
                "insert into client_balance (client_id, total, apply_at) values (?, ?, ?)",
                { new.client_id, new.delta, math.floor(fiber.time()) }
            )
        end

        return sql.execute(
            "update client_balance set total = ? where client_id = ?",
            { row.total + new.delta, new.client_id }
        )
    end
}
```

В этом примере `card_state` не изменяется потоком из OGG напрямую (её
состояние полностью определяется импортером `parquet`), но каждая операция из
OGG всё равно попадает в `journal_card_state` и вызывает триггер
`apply_delta`, который инкрементально ведёт баланс в `client_balance`.

### Системные настройки (`system`) {: #system_settings }

Секция `system` файла `config.yaml` позволяет настраивать системные параметры платформы "Кировец".

#### Настройка ресурсов импортеров {: #importer_resources }

Импортеры OGG, Dataflot и Parquet используют пул рабочих потоков для параллельной обработки файлов. По умолчанию размер пула равен `4` для каждого типа; при необходимости значение можно изменить независимо для каждого типа.

!!! note
    Параметры задают суммарный размер пула на тип импортера, а не на экземпляр. Например, десять сконфигурированных OGG-импортеров всё равно будут совместно использовать один пул из `importer_ogg_threads` потоков.

!!! warning
    Изменение значений параметров вступает в силу только после перезапуска сервиса Importer.

Доступные параметры:

- `importer_ogg_threads` — количество потоков для обработки файлов, поступающих от OGG (DSV-файлы). Значение по умолчанию: `4`.
- `importer_dataflot_threads` — количество потоков для обработки файлов Dataflot. Значение по умолчанию: `4`.
- `importer_parquet_threads` — количество потоков для обработки файлов в формате Parquet. Значение по умолчанию: `4`.
- `importer_memory_limit` — максимально допустимый процент занятой памяти сервиса Importer. При превышении этого порога обработка файлов приостанавливается до освобождения памяти. Значение по умолчанию: `80`.

Пример:

```yaml
system:
  importer_ogg_threads: 4
  importer_dataflot_threads: 4
  importer_parquet_threads: 4
  importer_memory_limit: 80
```
...
- `handler` (обязательно) — настройки обработчика запроса. Представляет
  ...
  - `args` — словарь, в которой ключ — имя аргумента запроса, а
    ...
    - `is_nullable` — если выставлено true, то аргумент не обязан
          ...
          необязательными
- `is_public` — если выставлено `true`, доступ к endpoint не требует авторизации (значение по умолчанию `false`)
...
??? example "config.yaml"
      ...
      system:                                         # секция системных настроек
        importer_memory_limit: 80                     # максимально допустимый процент занятости памяти сервиса Importer (0–100)
        importer_ogg_threads: 4                       # количество потоков для обработки OGG (DSV) файлов
        importer_dataflot_threads: 4                  # количество потоков для обработки Dataflot-файлов
        importer_parquet_threads: 4                   # количество потоков для обработки Parquet-файлов
        ...
        - path: kirovets/api/v1/app/lua                           # путь конечной точки. Может быть произвольным за исключением
          ...
          handler:
            ...
            name: datamarts.get_destination           # имя выставляемой функции. Для lua-функций указывается в формате <file_name>.<func_name>,
                                                      # где <file_name> — имя файла, расположенного в каталоге datamarts и содержащего код выставляемой функции <func_name>.
                                                      # (см. пример файла datamarts.lua ниже)
