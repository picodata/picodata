...
!!! warning "Использование `prune` совместно с фильтрами"
    Если на таблице настроен [фильтр](#filter_def), необходимо установить `prune: false`.
    Отфильтрованные записи считаются отсутствующими в снимке и при `prune: true` будут удалены,
    даже если это корректные данные.
...
```lua
{
    source     = string,         -- "ogg" | "dataflot" | "debezium-kafka" | "parquet"
