...
Добавьте в инвентарный файл дополнительные переменные `RADIX_LISTEN_ADDR`
...
Добавьте в инвентарный файл блок параметров, относящийся к Radix:
...
Если в кластере ранее была включена предыдущая версия плагина, то её
...
??? warning "Примечание"
    ...
    ```sql
    ALTER PLUGIN radix 0.14.1 SET radix.redis_compatibility = '{ "enforce_one_slot_transactions": true, "enabled_deprecated_commands": ["quit" ] }';
    ```
...
не явлется множеством, команда вернёт ошибку.
...
??? warning "Примечание"
    ...
    используйте следующий SQL-запрос:
    ```sql
    ALTER PLUGIN radix 0.14.1 SET radix.redis_compatibility = '{ "enforce_one_slot_transactions": true, "enabled_deprecated_commands": ["brpoplpush" ] }';
    ```
...
??? warning "Примечание"
    ...
    используйте следующий SQL-запрос:
    ```sql
    ALTER PLUGIN radix 0.14.1 SET radix.redis_compatibility = '{ "enforce_one_slot_transactions": true, "enabled_deprecated_commands": ["rpoplpush" ] }';
    ```
...
OBJECT ENCODING key
```
<font size="2">_поддерживается с версии 0.14.0_</font>
...
- удалена поддержа fedora:41 в связи с её EOL
...
- добавим по тесткейсу на каждый процент перфоманса
...
- перевели сборку Радикса на just
...
- Создаём тикет на обновление доков Радикса, а не аргуса
