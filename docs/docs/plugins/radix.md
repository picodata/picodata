...
### Обновление с предыдущей версии {: #plugin_upgrade }

Миграция с версий, ниже чем 1.0.0, невозможна. Функция переноса данных из
кластера из одного кластера Radix в другой находится в разработке.

### Перенос данных из Redis в Radix {: #migrate_from_redis }
...
- **default_user_name** — имя пользователя по умолчанию. Вариант `{
  "state": "on", "default_user_name": "<str>" }` используется для
