...
Sirin поддерживает те же типы данных, что и Cassandra. Ниже приведены поддерживаемые
типы:
...
- `time` — представление времени с точностью до наносекунд от начала суток
- `date` — количество дней, начиная с 01.01.1970
...
- `keyspace_name` — необязательный префикс, если выбрано ключевое пространство по умолчанию оператором `USE`
...
- необходимо указать хотя бы полный **partition key**
- допускается уточнять кластеризующие столбцы
- можно использовать операторы =, <, >, <=, >=, != и IN.
  CONTAINS, CONTAINS KEY на данный момент не поддерживаются
...
В дополнение к базовым операторам CQL, плагин Sirin поддерживает стандартные скалярные функции Cassandra.
