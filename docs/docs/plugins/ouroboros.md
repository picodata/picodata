...
- имя отдельного пользователя (должно совпадать на кластере-источнике и
  кластере-приёмнике), под которым Ouroboros будет подключаться
...
ouroboros:
  producer: # настройки кластера-источника (left)
    ...
    version_url: "http://<source_address>/ouroboros/api/v1/version"
    cluster_name: left
  consumer: # настройки кластера-приёмника (right)
    ...
    attributes:
      ...
      version_url: "http://<destination_address>/ouroboros/api/v1/version"
      cluster_name: right
...
```sql
CREATE PLUGIN ouroboros 1.5.2;
ALTER PLUGIN ouroboros 1.5.2 SET migration_context.tier='default';
ALTER PLUGIN ouroboros MIGRATE TO 1.5.2;
ALTER PLUGIN ouroboros 1.5.2 ADD SERVICE ouroboros TO TIER default;
ALTER PLUGIN ouroboros 1.5.2 ENABLE;
```
