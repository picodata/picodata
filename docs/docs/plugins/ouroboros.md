...
Плагин Ouroboros используется для однонаправленной асинхронной
логической [репликации](../overview/glossary.md#replication) данных
между двумя кластерами СУБД Tarantool. Плагин предназначен для
использования в особых случаях — например, когда требуется перенести
кластер Tarantool на новую площадку или заменить элементы инфраструктуры
без простоя.
...
1. Источником может быть только кластер Tarantool Cartridge, где
   развернуты [роли-сайдкары], включённые в поставку.
...
!!! note "Примечание"
    Плагин Ouroboros не предназначен для обеспечения
    отказоустойчивости кластера Picodata с помощью геораспределения. Эти
    задачи следует решать с помощью основной функциональности
    Picodata.

## Принцип работы {: #details }

Для использования Ouroboros требуются два кластера:
кластер-источник и кластер-приёмник. Далее необходимо [развернуть кластер]
Picodata с включённым плагином Ouroboros. В этом кластере может быть как
...
будет переносить данные из кластера-источника в кластер-приёмник с помощью
...
!!! note "Примечание"
    ...
    этих пользователей должны быть указаны в настройках плагина в
    соответствующих полях `username` и `password`.
...
бакетов>. Таким образом, получается параллельная отправка в
кластер-приёмник записей из различных бакетов. При этом сохраняется
...
- имена пользователей и их пароли для подключения к кластеру-источнику и
  кластеру-приёмнику
- группа параметров для подключения к кластеру-источнику (producer) и
  кластеру-приёмнику (consumer)
- группа параметров по настройке репликации (группы шардирования, исключаемые
  таблицы (спейсы), настройки параллелизма, переподключения и т.д.)
...
??? example "manifest.yaml"
        ...
        default_configuration:
          ...
          consumer:
            ...
            attributes:
              ...
              username: superadmin
              password: Pa55w0rd
              disabled_replicasets: ["group1-s3"]
    ...
    - migrations/0004_new_state_buckets.db
    ```
...
```yaml
ouroboros:
  producer: # настройки кластера-источника (left)
    cluster_name: left
    address:
      - localhost:3311
    username: superadmin
    password: Pa55w0rd
    disabled_replicasets: []
  consumer: # настройки кластера-приёмника (right)
...
Однако, для полноценного использования плагина рекомендуется запустить кластер с помощью [роли Ansible].
...
2. Скачайте нужную версию плагина `ouroboros` и положите пакет в рабочую
   директорию.
3. Проверьте наличие конфигурационного файла для плагина
   `ouroboros-config.yml`, проверьте настройки в нём (см. ниже).
...
## Управление плагином в командной строке {: #cli_mgmt}

Используйте поставляемую вместе с плагином Ouroboros утилиту командной
строки `ouroboros-cli` для управления плагином с текущего инстанса
кластера Picodata.

Для подключения к кластеру с запущенным плагином Ouroboros используйте следующий формат:

```shell title="Шаблон использования команды"
ouroboros-cli --host localhost --port 15001 --user superadmin --password Pa55w0rd <command>
```

На месте <command> используйте одну из следующих поддерживаемых команд:
...
Процесс работы плагина Ouroboros удобно отслеживать в графическом
...
кластере-источнике и кластере-приёмнике.
