...
[инстансом](../overview/glossary.md#instance) Picodata. Не имеет
значения, на каком именно инстансе кластера запущен этот веб-сервер; в
любом случае веб-сервер будет отображать данные обо всем кластере.

Функциональность веб-сервера по умолчанию включена в Picodata. Адрес, по
которому сервер доступен для клиентов, определяется параметром
конфигурации [instance.http.listen]. Его можно установить как в файле
...
параметра [instance.http.listen] (например,
[`127.0.0.1:5327`](http://127.0.0.1:5327)). Откроется страница
аутентификации:
