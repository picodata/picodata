...
## Автоматическое назначение имён {: #naming_convention }

Если [имя инстанса][instance_name] не задано администратором,
то при добавлении узлов в кластер Picodata автоматически назначит имя,
используя следующую схему:

`<имя_тира>_<номер_репликасета>_<номер инстанса>`

Например,`default_6_2` означает, что инстанс:

- имеет номер 2 в своём репликасете
- входит в репликасет номер 6
- репликасет, в котором находится инстанс, входит в тир с именем `default`.
...
кластера](deploy.md#multi_tier_cluster). Добавим к нему ещё один инстанс
