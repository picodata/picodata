...
## Запуск с использованием Docker {: #run_docker }

По умолчанию Picodata использует встроенный конфигурационный файл, где параметр
`listen` установлен в `0.0.0.0`. Это гарантирует корректную работу в контейнерной
сети.

Для использования собственной конфигурации можно смонтировать файл с хостовой
системы в контейнер. Такой файл будет доступен внутри контейнера по заданному
пути и позволит изменить настройки без пересборки образа. При запуске со
смонтированным конфигурационным файлом Picodata использует его вместо
встроенного конфигурационного файла.

### Запуск со стандартным файлом конфигурации {: #run_docker_default }

```bash
docker run picodata:latest
```

### Запуск с переопределенным файлом конфигурации {: #run_docker_overwritten }

В рамках примера, используемый файл конфигурации находится в текущей директории
и называется `picodata.conf`.

```bash
docker run \
  -v ./picodata.conf:/etc/picodata/config.yaml:ro \
  picodata:latest \
  run --config /etc/picodata/config.yaml
```

Параметры командной строки имеют приоритет выше файла конфигурации и могут
переопределять отдельные значения без замены всего файла. Больше информации
о порядке применения аргументов можно найти в [соответствующем разделе].

[соответствующем разделе]: ../reference/config.md#params_priority
...
Убедитесь, что у вас установлены `wget`, `docker`, `docker-compose`, а также что
...
из 4-х инстансов, а также конфигурационные файлы инстансов:

[docker-compose.yml]: https://git.picodata.io/core/picodata/-/raw/master/docker/docker-compose.yml?inline=false

```bash
wget https://git.picodata.io/core/picodata/-/raw/master/docker/{docker-compose.yml,picodata-1-1.conf,picodata-1-2.conf,picodata-2-1.conf,picodata-2-2.conf}
```
...
Для авторизации в [веб-интерфейсе](webui.md) Picodata также используйте учётную
запись администратора (логин `admin` и ранее заданный пароль).
