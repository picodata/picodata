...
Максимальное количество неуспешных попыток аутентификации. Каждая
неуспешная попытка аутентификации увеличивает счетчик
...
также регулировать потребление сетевых и вычислительных ресурсов.
...
ограничение на максимальное количество порталов (`pg_portal_max`) и
...
`pg_statement_max` = 1000, 200 клиентов смогут одновременно иметь не
...
Максимальное количество SQL-запросов, которые могут одновременно
выполняться на одном инстансе.

Параметр помогает сдерживать рост потребления памяти во время выполнения
запросов (например, за счет временных объектов) и ограничивать нагрузку в
сценариях, где часть запросов ожидает данные с диска.

Значение по умолчанию: `50`

Допустимые значения: целое число, не меньше `1`

### sql_storage_cache_count_max

Максимальное количество prepared statement-ов для размещения в LRU-кеше
...
Динамический параметр Box API, регулирующий размер LRU-кеша в байтах.
...
Семантика параметра совпадает с одноимённой опцией DQL. В частности, при
`read_preference = replica` полностью локальный запрос выполняется через
local fast-path только если он пришёл на реплику целевого репликасета
(или локального tier). Если такой же запрос приходит на лидер, Picodata
маршрутизирует его на реплику по обычному пути.

Значение по умолчанию: `leader`

### sql_ddl_timeout

Таймаут по умолчанию (в секундах) для DDL и ACL операций
(CREATE TABLE, ALTER TABLE, GRANT и др.), если в SQL-запросе
не указан явный `OPTION(TIMEOUT = N)`. Поддерживает дробные
значения (например, `3.5`).

Значение по умолчанию: `86400` (24 часа)

### sql_log

Включает логирование SQL-запросов в лог-файл.

!!! warning title "Примечание 1"
    ACL запросы не логируются в целях дополнительной безопасности.

!!! warning title "Примечание 2"
    Следует использовать с осторожностью, т.к. залогированные запросы могут содержать конфиденциальные данные и даже содержать пароли в открытом виде.
