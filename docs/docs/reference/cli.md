...
Следующие команды поддерживают подключение к инстансу с настроенным mutual TLS (mTLS):
...
Признак использования аутентификации по сертификату при доступе к инстансу, на
...
сертификату, закрытому клиентскому ключу и корневому сертификату).

Аналогичная переменная окружения: `PICODATA_IPROTO_TLS_AUTH`

### --tls-ca {: #tls_ca }

Путь к файлу корневого сертификата для доступа к инстансу, на котором
включен и настроен mTLS.

`--tls-ca <CA_FILE>`

Аналогичная переменная окружения: `PICODATA_IPROTO_TLS_CA`

### --tls-cert {: #tls_cert }

Путь к файлу клиентского сертификата для доступа к инстансу, на котором
включен и настроен mTLS.

`--tls-cert <CERT_FILE>`

Аналогичная переменная окружения: `PICODATA_IPROTO_TLS_CERT`
