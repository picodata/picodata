...
Типизированный одномерный массив. Доступные типы элементов: [BOOLEAN](#boolean), [DATETIME](#datetime), [DECIMAL](#decimal), [DOUBLE](#double), [INTEGER](#integer), [JSON](#json), [TEXT](#text), [UUID](#uuid).

!!! note "Примечание"
    На данный момент массивы нельзя сравнивать и использовать в качестве ключей шардирования.
...
!!! note "Примечание"
    На данный момент поддерживается только 4-я версия UUID.
