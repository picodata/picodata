...
- `CONST-FILTERED STORAGE, N/M` — для запроса удалось вычислить конкретные
  ...
  количество таких репликасетов и количество всех доступных репликасетов в кластере.
- `DYN-FILTERED STORAGE` — репликасеты, на которые надо отправить запрос, определяются
