...
Данная команда требует привилегии `WRITE TABLE`.

Для варианта `INSERT ... ON CONFLICT (...) DO UPDATE` см.
[Вставка с обновлением при конфликте](iocdu.md#required_privileges).
...
`ON CONFLICT`. На данный момент поддерживаются следующие режимы:

- `DO FAIL`, вернуть ошибку в случае конфликта
- `DO REPLACE`, затереть старый кортеж новым по первичному ключу
- `DO NOTHING`, ничего не делать (оставить старую версию кортежа)
- `DO UPDATE`, вставить новый кортеж или, при конфликте по явно
  указанному списку колонок, обновить существующий. Подробности и
  ограничения см. в разделе
  [Вставка с обновлением при конфликте](iocdu.md)
...
Вариант с `DO UPDATE` реализует вставку с обновлением при конфликте. Он
требует явный список колонок в `ON CONFLICT` и позволяет обновлять
существующий кортеж по первичному ключу или уникальному индексу. Этот
режим поддерживается только внутри транзакционных блоков для
шардированных таблиц, поэтому детали и ограничения вынесены в отдельную
страницу [Вставка с обновлением при конфликте](iocdu.md).

Например, для таблицы `account_balances(id, amount)`:
...
!!! note "Примечание"
    При вставке в таблицу большого числа строк может сработать
...
в `decimal 2.5`). В параметризированном виде дробному числу будет
