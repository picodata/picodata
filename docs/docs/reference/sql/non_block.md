...
Ситуация с задержками и/или блокировкой транзакционного потока актуальна для:
- DQL-запросов (`SELECT`)
- DML-запросов (`CALL`, `DELETE`, `INSERT`, `UPDATE`)
- использования этих команд в подзапросах
...
Параметры `sql_preemption` и `sql_preemption_interval_us` отвечают за
отзывчивость выполнения, но не ограничивают количество одновременно
исполняемых SQL-запросов.

Для ограничения параллельности используйте параметр
[sql_runtime_concurrency_max](../db_config.md#sql_runtime_concurrency_max):
он задает верхний предел числа SQL-запросов, одновременно работающих на
инстансе.
...
ALTER SYSTEM SET sql_runtime_concurrency_max = 4;      -- ограничить параллельность
```
...
Чтобы избежать чрезмерной нагрузки и роста промежуточных данных,
используйте ограничения двух типов.

Через `ALTER SYSTEM`:

- [sql_runtime_concurrency_max](../db_config.md#sql_runtime_concurrency_max),
  ограничение на максимальное количество SQL-запросов, одновременно
  выполняющихся на инстансе.

Через `OPTION(...)` в запросах:
