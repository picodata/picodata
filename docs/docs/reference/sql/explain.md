...
2. Query (ROUTER):
SELECT "COL_0" as nmbr, "COL_1" as product, "COL_2" as quantity FROM ( SELECT "COL_0", "COL_1", "COL_2" FROM "_tmp_14401666969040046443_0136" ) ORDER BY 1 LIMIT 1
+----------+-------+------+-----------------------------------------------------------+
| selectid | order | from | detail                                                    |
+=====================================================================================+
| 0        | 0     | 0    | SCAN TABLE _tmp_14401666969040046443_0136 (~1048576 rows) |
|----------+-------+------+-----------------------------------------------------------|
...
FROM
  ...
  LEFT JOIN (
    SELECT
      ...
      "COL_4"
    FROM
      "_tmp_6553864902677354202_1136"
  ) as orders ON items."COL_2" = orders."COL_2"
+----------+-------+------+----------------------------------------------------------+
| selectid | order | from | detail                                                   |
+====================================================================================+
| 1        | 0     | 0    | SCAN TABLE _tmp_6553864902677354202_1136 (~1048576 rows) |
|----------+-------+------+----------------------------------------------------------|
| 0        | 0     | 0    | SCAN TABLE warehouse (~1048576 rows)                     |
|----------+-------+------+----------------------------------------------------------|
| 0        | 1     | 1    | SCAN TABLE _tmp_6553864902677354202_0136 (~1048576 rows) |
|----------+-------+------+----------------------------------------------------------|
...
Если узла `motion` в плане нет, это тоже значимая информация:
планировщик доказал, что соответствующее поддерево может быть выполнено
локально, без межшардового обмена данными. Это возможно, например, если
это поддерево направлено в один бакет или работает только с глобальными
таблицами.
...
Если фильтр определяет значения всех колонок ключа шардирования,
планировщик может вычислить один бакет и удалить глобальные
reduce-стадии даже для
`DISTINCT`, `GROUP BY`, агрегатов, `HAVING`, `ORDER BY ... LIMIT`,
подзапросов и `WITH` в соответствующем поддереве плана. Если нужная
часть запроса затрагивает несколько бакетов или это невозможно доказать
на этапе планирования, в план добавляются узлы `motion`.

Примеры разных вариантов `motion policy` приведены ниже.

### Отсутствие перемещения {: #no_motion }

Отсутствие узла `motion` означает, что соответствующее поддерево плана
может быть исполнено локально. Для шардированных таблиц это обычно
значит, что фильтр определяет значения всех колонок ключа шардирования,
и планировщик вычислил один конкретный бакет.
...
В примерах ниже используется шардированная таблица `t`, распределенная
по колонке `a`.

Пример одноузловой агрегации:

```sql
EXPLAIN SELECT sum(b), count(*), min(a) FROM t WHERE a = 1;
```

Существенная часть плана:

```sql
projection (sum("t"."b") -> "col_1", count(*) -> "col_2", min("t"."a") -> "col_3")
...
Контрпример для `ORDER BY ... LIMIT`:

```sql
EXPLAIN SELECT b FROM t WHERE b = 1 ORDER BY b LIMIT 1;
```
...
Здесь фильтр не определяет значение колонки шардирования `a`, так что
локальной может быть только часть вычислений. Планировщик выполняет
локальный `ORDER BY ... LIMIT` на каждом узле хранения, а затем
добавляет глобальный `motion [policy: full]` и верхний
`ORDER BY ... LIMIT`.
...
Если тот же класс операций можно ограничить одним бакетом, `motion
[policy: full]` может исчезнуть. Пример такого плана приведен в разделе
[Отсутствие перемещения](#no_motion).
...
как для локальных DML-запросов, так и для DQL-запросов, в которых
фильтр определяет значения всех колонок ключа шардирования. В примере
ниже номер
бакета выведен из условия (`id = 5`):
...
Тот же принцип действует и для `SELECT`: если `WHERE` однозначно
определяет значения всех колонок ключа шардирования, `EXPLAIN` покажет
конкретный бакет даже в том случае, когда в плане нет узла `motion`.
Пример см. в разделе [Отсутствие перемещения](#no_motion).
