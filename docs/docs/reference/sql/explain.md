...
- расчетный диапазон [сегментов](../../overview/glossary.md#segment), на
...
1. **Локальная вставка**. Представляет собой локальную [материализацию
   ...
   будут сгруппированы по этим сегментам). Перемещения данных на другие
...
## Варианты использования сегментов {: #segment_ranges }

Планировщик SQL-запросов в Picodata показывает, на каких сегментах будет
...
- номер сегмента (`[215]`)
- любой сегмент (`any`)
- неизвестный набор сегментов (`unknown`)
