...
храниться на нескольких узлах кластера. Планировщик может как прочитать
данные со всех подходящих узлов, так и выполнить отдельные поддеревья
плана на одном бакете, если соответствующие условия `WHERE` определяют
значения всех колонок ключа шардирования.
...
!!! note "Примечание"
    Если по условиям `WHERE` планировщик может доказать, что
    соответствующее поддерево плана выполняется строго в одном бакете,
    оно будет выполнено без глобального `motion`. В этом случае
    `DISTINCT`, `GROUP BY`, `HAVING`, `ORDER BY` и `LIMIT` также могут
    быть выполнены локально для этого поддерева. Для составного ключа
    шардирования это обычно означает, что в фильтре определены значения
    всех его колонок. Если доказать один бакет нельзя, Picodata построит
    распределенный план с межшардовым обменом данными для этой части
    запроса. Для анализа такого плана используйте команду
    [`EXPLAIN`](explain.md).
