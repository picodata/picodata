...
* **COLUMN** — имя колонки. Соответствует правилам имен для всех
  [объектов](object.md) в кластере. Для каждой колонки индекса `TREE`
  можно указать порядок сортировки: `ASC` (по возрастанию) или `DESC`
  (по убыванию). Если порядок не указан, используется `ASC`. Например,
  `(id DESC, name ASC)`. Указанный порядок определяет направление
  соответствующей части физического индекса. Для индексов `HASH`,
  `RTREE` и `BITSET` явный порядок сортировки не поддерживается
...
* **COMPRESSION_LEVEL** (`vinyl`) — уровень сжатия данных в индексе.
  Применяется Zstd со шкалой уровней в диапазоне от `-7` до `22`. По
  умолчанию используется уровень сжатия `3`. Значение `0` является
  служебным и означает "использовать сжатие по умолчанию"
...
* **TIMEOUT** — интервал времени, за который Picodata пытается создать
  ...
  вернется одна строка. Если параметр не указан, используется значение
  системного параметра [`sql_ddl_timeout`](../db_config.md#sql_ddl_timeout)
