...
[объектов](object.md) в кластере
    ...
    Для каждой колонки индекса `TREE` можно указать порядок сортировки:
    `ASC` (по возрастанию) или `DESC` (по убыванию). Если порядок не
    указан, используется `ASC`. Например, `(id DESC, name ASC)`.
    Указанный порядок определяет направление соответствующей части
    физического индекса. Для индексов `HASH`, `RTREE` и `BITSET`
    явный порядок сортировки не поддерживается.
