...
- Если в одном `INSERT ... ON CONFLICT (...) DO UPDATE` несколько входных
  строк имеют один и тот же конфликтный ключ, запрос завершится ошибкой.
...
Также можно ссылаться на `LET`-переменную, видимую в этой точке блока.
Переменная, объявленная в теле `IF`-блока, после `END IF` уже недоступна —
подробнее см. [`LET`-переменные](transactions.md#let_statement). Имя
переменной не должно совпадать с именами колонок, доступных в запросе:
