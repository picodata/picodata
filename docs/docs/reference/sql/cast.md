...
Для строковых констант также поддерживается синтаксис типизированного
литерала из PostgreSQL — имя типа перед строкой.
Запись `INT '42'` эквивалентна `CAST('42' AS INT)`:

```sql
sql> SELECT INT '42', BOOL 't';
+-------+-------+
| COL_1 | COL_2 |
+=======+=======+
| 42    | true  |
