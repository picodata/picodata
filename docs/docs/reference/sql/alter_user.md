...
* **CERT** — аутентификация пользователя по клиентскому сертификату при
  подключении по протоколу PostgreSQL, см. [Аутентификация по
  сертификату](../../admin/ssl.md#pgproto_cert_auth). Пароль не
...
!!! note "Примечание"
    Попытка отозвать разрешение на подключение к системе (**LOGIN**) у
    [системных пользователей] `admin` и `pico_service` вернет ошибку,
    так как это может привести к полной недоступности кластера.
