...
## Параметры и маршрутизация {: #routing }

Для шардированных таблиц параметры влияют не только на типизацию, но и
на маршрутизацию запроса. Если условие определяет значения всех колонок
ключа шардирования константами и/или параметрами, соответствующее
поддерево подготовленного плана может быть направлено на один бакет и
выполнено без глобального `motion`.

Например, пусть таблица `t` распределена по `a`. Тогда подготовленный
запрос
...
может быть выполнен без `motion`, потому что фильтр определяет значения
всех колонок ключа шардирования и планировщик может направить
выполнение в один бакет. А запрос
...
не может, потому что параметр ограничивает только колонку `b`, которая
не входит в ключ шардирования, а ключ шардирования `a` остается
неизвестным. В таком случае
Picodata сохраняет распределенный план; в `EXPLAIN` это обычно
проявляется reduce-stage с узлом `motion`.

Для составного ключа шардирования действует то же правило: чтобы
доказать один бакет, планировщику обычно нужно, чтобы фильтр определял
значения всех колонок такого ключа.
