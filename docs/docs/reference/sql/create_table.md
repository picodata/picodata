...
* **PRIMARY KEY** — первичный ключ. Обеспечивает уникальность и сортировку данных только
  ...
  Поддерживается специальный синтаксис с указанием первой колонкой системной колонки
  [bucket_id](../../architecture/distributed_sql.md#).
  В этом случае системная колонка добавляется в первичный ключ в префикс, а отдельный [служебный
...
Если параметр не задан, будет использован тир, для которого задано
значение `is_default=true` в [системной таблице](../../architecture/system_tables.md#_pico_tier) `_pico_tier`.
