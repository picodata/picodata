...
Так как а) записи в журнале не могут менять порядок и б) каждой записи соответствует только один индекс и терм, то указанные выше правила гарантируют консистентность журнала. Raft-журнал хранится в ОЗУ узла, на котором запускается инстанс.
...
- `'vinyl'` — дисковый движок хранения данных. Позволяет работать с БД, размер которой превышает размер ОЗУ. При этом, основная часть данных находится на диске, а в ОЗУ доступен лишь кеш часто используемых данных. Движок `'vinyl'` работает на основе журнально-структурированных деревьев со слиянием, или LSM-деревьев (Log Structured Merge Tree), которые показывают хорошую производительность при работе с твердотельными дисками (SSD). Последние отличаются высокими показателями чтения, особенно в сравнении с более медленной записью данных.
...
- **Индексы служебных колонок**<br>
  ...
  Индекс не создается, если при создании таблицы используется специальный
  синтаксис `PRIMARY KEY (bucket_id, ...)` — в этом случае системная колонка
  `bucket_id` добавляется в первичный индекс. Для нее также можно указать
  порядок сортировки `ASC` или `DESC`, например
  `PRIMARY KEY (bucket_id DESC, ...)`.
...
Режим синхронизации WAL настраивается параметром [`tier.wal_mode`](../reference/config.md#cluster_tier_tier_wal_mode). Этот параметр задается на уровне [тира](#tier), поскольку именно тиры в Picodata служат для разделения данных по степени важности: одни данные критичны и требуют строгих гарантий сохранности, другие — менее ценны и допускают компромисс в пользу производительности. Узлы одного тира обслуживают однородные по важности данные, поэтому различать `wal_mode` между узлами внутри тира смысла нет — выбор режима естественно делается для тира целиком.

По умолчанию используется значение `write`: файбер, обрабатывающий транзакцию, дожидается записи данных в журнал через `write(2)`, но не вызова `fsync(2)`, после чего управление возвращается пользователю. Данные при этом попадают в страничный кеш ОС, но не гарантированно на диск — при сбое питания или ОС возможна потеря или повреждение последних записей. Если данные к этому моменту уже были переданы на другие инстансы за счет [репликации](#replication), они не теряются даже без `fsync` на исходном узле — подробнее о гарантиях сохранности см. раздел про репликацию.

Для надежного восстановления после аппаратного сбоя или отказа ОС рекомендуется устанавливать режим `fsync`. В этом режиме для каждой записи журнала гарантируется синхронизация c диском (fsync) до передачи управления обратно пользователю.

| Режим   | `write(2)` | `fsync(2)` | Надежность                       | Производительность |
|---------|:----------:|:----------:|----------------------------------|--------------------|
| `write` | да         | нет        | потеря при сбое питания/ОС       | высокая            |
| `fsync` | да         | да         | защита от отказа (рекомендуется) | ниже               |
...
### Состояние инстанса {: #state }

**Состояние инстанса** — оценка доступности [инстанса](#instance) в Raft-группе. Состояние отражает то, как инстанс сконфигурирован его соседями. Существуют текущий (`current`) и целевой (`target`) типы состояний. За приведение первого ко второму отвечает [governor (губернатор)](#governor).
...
разворачиваются на каждом его узле. Управление жизненным циклом плагина
...
формате YAML (manifest.yaml) с описанием плагина, списком его сервисов и
...
Компактизация — процесс оптимизации журнала записей. Применительно к
Picodata, компактизация может встречаться в следующих контекстах:

**Компактизация raft-журнала (raft log compaction)** — процесс, не
допускающий бесконтрольного роста журнала записей Raft. Компактизация
заключается в удалении части журнала, относящейся к сделанному ранее
[снапшоту](#snapshot).

**Компактизация LSM-дерева** — процесс эффективного управления
хранилищем "ключ-значение" на диске при использовании [движка хранения](#db_engine)
Vinyl. Обслуживание LSM-дерева подразумевает удаление устаревших и
дублирующихся записей и оптимизацию операций чтения.

### Создание снапшотов (snapshotting) {: #snapshotting }

**Создание снапшотов** — процесс периодического сохранения состояния [инстанса](#instance) на жесткий диск. Наличие [снапшотов](#snapshot) позволяет восстановить инстанс в прежнем виде после его перезапуска.
...
Governor управляет жизненными циклами инстансов, реагируя на изменения их [состояний](#state).
...
другой, выполняющейся в то же время. _Compare and swap_ решает эту
...
- внутри [репликасета](#replicaset) — распространяет внутри репликасета операции над шардированными таблицами.
- глобальная на весь [кластер](#cluster) — распространяет по всему кластеру изменения в пользовательских
  и [системных](../architecture/system_tables.md) глобальных таблицах.
...
В зависимости от типа синхронизации изменений внутри репликасета:

- **синхронная**. Запись в таблицу [тира](#tier) происходит только тогда, когда
  эта операция была подтверждена кворумом репликасета (попала в
  WAL-журнал инстансов).
- **асинхронная**. Запись в таблицу [тира](#tier) происходит асинхронно — не дожидаясь подтверждения от реплик.
...
- **Repeatable read**. Повторяемое чтение данных, при котором нельзя изменять данные до тех пор, пока читающая их транзакция не завершилась.
...
### Машрутизация запросов с учётом шардирования данных {: #shard_aware_routing }

Когда пользователь выполняет SQL-запрос, Picodata, как распределённая
СУБД, самостоятельно и без вмешательства пользователя производит
внутренние вычисления и определяет, [на каких узлах находятся нужные
данные](../architecture/distributed_sql.md#data_distribution). Внутренние
запросы между узлами кластера выполняются автоматически. Это позволяет
клиенту не задумываться о выборе [репликасета](#replicaset) для
выполнения запроса, а делегировать это СУБД.

В то же время, обработка запроса может добавить сетевую задержку и
увеличить время его выполнения:

- клиент посылает запрос
- запрос попадает на узел А, к которому подключён клиент
- если узел А не содержит данные, которые запрашивает клиент:
    - Picodata перенаправляет запрос по сети на узел Б (с данными)
    - узел Б (с данными) обрабатывает запрос и возвращает ответ на узел А
    - узел А отдаёт ответ клиенту
- если узел А содержит данные, которые запрашивает клиент:
    - узел А обрабатывает запрос и отдаёт ответ клиенту

В случаях, когда необходима максимальная производительность, определять
нужный репликасет можно на стороне внешнего коннектора за счёт
реализации механизма **shard-aware routing**: коннектор должен запросить
дополнительную информацию о запросе у Picodata и тогда по ней он сможет
вычислить _bucket_id_ и отправить запрос сразу на репликасет, отвечающий
за хранение соответствующей порции данных. В основе вычисления
_bucket_id_ лежат _метаданные запроса_:

- информация о [тире](#tier), к которому относится таблица
- список и порядок параметров (данных) запроса, по которым необходимо
  приводить вычисления (Picodata формирует это на основе [ключей
  распределения](#sharding_key) таблицы, а порядок берёт из запроса)
- [типы данных](../reference/sql_types.md), участвующих в вычислении.

Picodata реализует только передачу этих метаданных по запросу от
клиента. Непосредственно маршрутизацию запросов на основе этих метаданных
каждый клиент релизует сам. К примеру, это реализуют наши коннекторы
[Picopyn](../dev/connectors/picopyn.md) и [Rust](../dev/connectors/rust.md).
...
Запрос (и, опционально, значения передаваемых параметров) к базе данных на языке SQL.

### План запроса {: #execution_plan }

Схема и последовательность шагов, требуемая для исполнения запроса. План строится на узле-маршрутизаторе и исполняется во всем кластере.

### Строгая функция {: #strict_function }

Если хотя бы один аргумент равен `NULL`, функция вернёт `NULL` без каких-либо вычислений.
...
Строка таблицы или результата запроса, представленная как упорядоченный набор значений.
...
## Подготовленный запрос {: #prepared_statement }

Prepared Statement (Подготовленный запрос) — это механизм в СУБД для
многократного выполнения SQL-запросов с параметрами, который заранее
компилирует шаблон запроса, а затем подставляет данные, что повышает
производительность (за счет кеширования) и, самое главное, надежно
защищает от SQL-инъекций, разделяя код запроса и данные.

## Транзакции {: #transactions }

Транзакция — это логическая группа
[запросов](#query) к базе данных (`SELECT`, `UPDATE`...),
выполняемая как единое целое: либо успешно фиксируются все изменения
(COMMIT), либо, при ошибке, все действия полностью отменяются
