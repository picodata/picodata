...
### Хранение конфигурации {: #config_storage }

Конфигурация Picodata — это совокупность параметров отдельных инстансов,
тиров и кластера в целом. Конфигурация хранится в:

- [системных таблицах] БД, которые, соответственно, размещаются в ОЗУ
физических или виртуальных хостов, на которых выполняются голосующие
инстансы. Этот набор глобальных таблиц составляет runtime-конфигурацию,
которая поддерживается с помощью алгоритма Raft в актуальном и
консистентном состоянии между всеми узлами кластера.
- [рабочей директории инстансов] (задается при запуске инстанса) в файлах
  `*.snap` (полный снимок данных инстанса) и *`.xlog`/`*.vylog` (история
  изменения журнала WAL)
- [файлах конфигурации], использованных при запуске инстансов
- [переменных окружения] (при их использовании) при запуске инстансов на соответствующих узлах

Большая часть параметров кластера задается один раз при его
первоначальном развертывании и в дальнейшем не меняется. Некоторые
параметры (например, настройки памяти, уровень журналирования,
размещение рабочих файлов и т.д.) динамичны и могут быть переопределены
при повторных запусках инстансов. Более подробно это описано в разделах
[Конфигурирование] и [Справочник настроек].

[системных таблицах]: ../architecture/system_tables.md
[рабочей директории инстансов]: ../architecture/instance_runtime_files.md
[файлах конфигурации]: ../reference/config.md
[Конфигурирование]: ../admin/configure.md
[Справочник настроек]: ../reference/settings.md

На уровне файловой системы, конфигурация кластера — это рабочие файлы инстансов,
входящих в его состав, а также все остальные файлы, которые были использованы
для их запуска — например, файлы конфигураций (`*.yaml`), скриптов (`*.sh`) и т.д. В
этом понимании конфигурация Picodata будет содержаться в [резервной копии
кластера], с помощью которой кластер можно запустить заново с сохранением его
прежней конфигурации.

[переменных окружения]: ../admin/configure.md#use_env
[резервной копии кластера]: ../admin/backup_and_restore.md
...
- [Сценарии использования Picodata](use_cases.md)
