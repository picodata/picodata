...
#### Использование контекста миграции {: #migration_context }

##### Установка параметров {: #migration_context_setup }

В миграциях можно ссылаться на значения параметров, установленных
в пространстве имен `migration_context`.
...
задание [тиров][tier], на которых будут
[созданы](../reference/sql/create_table.md) нужные для работы плагина
таблицы.
...
указать их имена в контексте миграции плагина. Посредством подстановки, имя
тира в кластере становится доступным в миграции.

##### Валидация параметров {: #migration_context_validate }

Корректность параметров, устанавливаемых для контекста миграции, можно
проверять с помощью валидаторов. Они задаются в функции, помеченной
атрибутом `#[migration_context]`, единственным параметром которой
является мутабельная ссылка на структуру данных `MigrationValidator`.

??? example "Пример использования атрибута `migration_validator`"
    ```rust
    #[migration_validator]
    pub fn migration_validator(mv: &mut MigrationValidator) {
        mv.set_context_validator(|ctx| {
            if ctx.len() == 3 {
                Err("context length is not valid".into())
            } else {
                Ok(())
            }
        });
        ...
        mv.set_context_parameter_validator(|k, v| match k.as_str() {
            "hot_tier" => if tier_is_valid(k, v) {
                Ok(())
            } else {
                Err(format!("tier {k} has invalid value {v}").into())
            },
            _ => Ok(()),
        });
    }
...
Для проверки отдельных параметров нужно задать валидатор методом
`set_context_parameter_validator` вышеупомянутой структуры.
Заданный валидатор вызывается при выставлении параметров контекста
миграции, например:

```sql
ALTER PLUGIN myplugin 0.1.0 SET migration_context.hot_tier='tier_2';
```

Перед сохранением значения в конфигурацию `myplugin`, будет проведена
валидация параметра `hot_tier` со значением `tier_2`. Если валидатор
вернет ошибку — значение не сохранится, а сообщение ошибки будет
показано пользователю.

Также, можно задать валидатор всего контекста миграции с помощью
метода `set_context_validator`. Заданный валидатор вызывается при
миграции плагина, например:

```sql
ALTER PLUGIN myplugin MIGRATE TO 0.1.0;
```

Перед применением миграции, будет вызван валидатор всего
контекста миграции с единственным аргументом — переменной типа `HashMap`,
в которой имени параметра сопоставляется его значение. Если валидатор
вернет ошибку — миграция не произойдет, а сообщение ошибки будет показано
пользователю.

По умолчанию валидаторы выключены, так что задание функции с атрибутом
`#[migration_context]` опционально.
...
библиотекой `picodata-plugin`, которая предоставляет trait Service и макрос
`#[service_registrar]`.
