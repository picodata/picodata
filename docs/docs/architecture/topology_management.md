...
Важной составляющей топологии является информация о **состоянии** всех
узлов. Данный термин в  Picodata означает не самооценку инстанса, а
конфигурацию остальных участников кластера по отношению к нему.
Существуют две разновидности состояний: текущее (`current_state`) и
целевое (`target_state`).

Управление топологией в Picodata происходит за счет последовательного
изменения состояний: сначала меняется значение `target_state`,
затем оно передается в `current_state`.
...
Все изменения состояний в кластере регистрируются в [журнале
...
- Инстанс отправляет raft-лидеру запрос [`.proc_raft_join`], содержащий
    - его сетевой адрес `instance.iproto.advertise`
...
Если параметр `replicaset_name` не передан явно, репликасет для нового
инстанса выбирается автоматически:

- начиная с версии 26.2: если имя инстанса соответствует стандартной схеме
  генератора имен, то есть `<имя_тира>_<номер_репликасета>_<номер_инстанса>`
  (например, `default_6_2`), и при этом репликасет
  `<имя_тира>_<номер_репликасета>` существует, не помечен на удаление и
  относится к тиру `<имя_тира>` — инстанс присоединяется к этому репликасету;
- иначе — выбирается первый подходящий репликасет: с достаточным
  количеством свободных мест (согласно [фактору
  репликации](../overview/glossary.md#replication_factor)), без конфликтов
  по [домену отказа](../overview/glossary.md#failure_domain) и не в процессе
  [выхода из кластера](../tutorial/node_expel.md#expel); если такого
  репликасета нет, создаётся новый.
...
времени, заданного системным параметром [`governor_auto_offline_timeout`]
(по умолчанию, 30 секунд).

[`governor_auto_offline_timeout`]: ../reference/db_config.md#governor_auto_offline_timeout

В случае недоступности мастера репликасета происходит аварийное
переключение и новым мастером становится одна из резервных реплик. Если
в дальнейшем связь с прежним мастером восстановится, он вернется в
состав репликасета уже как резервная реплика (т.е., нового переключения
не будет).

## Governor — централизованное управление кластером {: #governor }

Большинство механизмов, связанных с отказоустойчивостью в Picodata, так
или иначе связаны с компонентом governor. С технической точки зрения
governor — это поток управления (fiber), всегда выполняющийся на
[raft-лидере](../overview/glossary.md#raft_leader). Губернатор действует
в два этапа. По данным из системных таблиц, которые отражают состояние
кластера, генерируется событие. Например, лидер некоторого репликасета
имеет `target_state` = `Offline`. Второй этап — реакция на событие, при
этом все необходимые действия предпринимает также губернатор. Например,
в случае недоступности лидера репликасета, необходимые вызовы RPC и
записи в глобальные таблицы будет делать непосредственно губернатор.

### Доступность инстансов {: #instance_reachability }

Инстанс называется _доступным_ или логически _доступным_, если его
целевое состояние (`target_state`) не равно `Offline` или `Expelled`.
...
Доступность инстансов влияет на возможность изменять содержимое таблиц в
кластере. При плановом выключении одного из серверов кластера, или при
аварии в одном из датацентров инстансы могут уходить в состояние `Offline`.
В этом случае [DDL-операции](../reference/sql/ddl.md) в кластере
перестают работать, даже если реплики/мастера этих инстансов в соответствующих
репликасетах остались доступны. Возможность проводить кластерные
DDL-операции возобновится после того, как неактивные инстансы перейдут в
состояние `Online` или `Expel`. До этого момента DDL-операции будут
доступны только в локальном режиме (с параметром `WAIT APPLIED
LOCALLY`).

### Управление состоянием инстансов {: #instance_health_checks }

В Picodata реализованы дополнительные средства проверки состояния
инстанса. Помимо признака доступности, у каждого инстанса имеются
дополнительные поля в системной таблице `_pico_instance`:

- `target_state_reason` — причина перехода в целевое состояние
- `target_state_change_time` — время последнего изменения target_state

Основная задача этих полей — помощь при отладке недоступности инстанса
(например, если инстанс была автоматические переведен в состояние
`Offline` алгоритмом [`sentinel`]).

Часто встречающиеся значения поля `target_state_reason`:

| Причина | Описание |
| ---      | ---      |
| wakeup | инстанс присоединился к репликасету после периода недоступности
| join | инстанс присоединился к репликасету |
| expel | инстанс был выведен из кластера командой `picodata expel` |
| shutdown | инстанс был выключен (процесс инстанса завершен) |
| replicaset expel | последний инстанс репликасета получил `target_state = Expelled`, а сам репликасет — `ToBeExpelled` |
| replication error + подробности | инстанс недоступен и переведен в состояние `target_state = Offline` из-за [ошибки репликации] данных внутри репликасета |
| auto-online | из-за конфликта механизма CaS инстанс получил `target_state = Offline`, что не соответствует действительности. Инстанс автоматически возвращает себе `target_state = Online` с данной причиной |

[ошибки репликации]: ../reference/db_config.md#governor_check_replication_error

См. также:

- [Системная таблица _pico_instance](../architecture/system_tables.md#_pico_instance)
