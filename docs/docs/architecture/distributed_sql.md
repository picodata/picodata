...
- Picodata SQL работает с шардированными
  ...
  [сегментам](../overview/glossary.md#segment)
- Vshard работает с распределением сегментов по
...
сегмента) от функции шардирования (на вход подается набор колонок, по
которым происходит шардирование, и количество сегментов в кластере). Для
...
`sharding_key`, попадут в один сегмент.
