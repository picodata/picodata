...
В рамках одного SQL-запроса разные поддеревья плана могут исполняться по
разным сценариям. В зависимости от того, может ли планировщик
однозначно вычислить бакет по ключу шардирования для конкретного
поддерева, оно исполняется по одному из двух сценариев:

- **Одноузловое поддерево (`single-node`)**. Если при анализе фильтров в
  поддереве планировщик может доказать, что оно исполняется строго в
  одном бакете, такое поддерево направляется на один репликасет. В этом
  случае не добавляются узлы межшардового перемещения данных (`motion`)
  и не требуется глобальная стадия консолидации.
- **Распределенное поддерево**. Если соответствующая часть запроса
  затрагивает несколько бакетов либо это нельзя доказать на этапе
  планирования, Picodata опрашивает несколько узлов хранения и при
  необходимости добавляет стадии `motion` и последующей консолидации
  результатов.

Одноузловый режим возможен не только для простого чтения по ключу
шардирования, но и для `DISTINCT`, агрегатных запросов, `GROUP BY`,
`ORDER BY ... LIMIT`, подзапросов и `WITH`, если все используемые данные
гарантированно остаются в одном бакете. В простых запросах такое
поддерево может совпадать со всем запросом целиком.
...
кластере с одним координатором запросов.
