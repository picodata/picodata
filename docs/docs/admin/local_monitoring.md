...
## Получение данных по HTTP API {: #instance_health_check }

Инстанс Picodata отдаёт сведения о своём состоянии и работоспособности
по HTTP API на ряд эндпоинтов, описанных ниже. HTTP-код `200` означает
...
### Параметры инстанса и кластера {: #status_endpoint }
...
|:------------------------:|:--------------------------------------------:|:------------:|:------------:|
| `/api/v1/health/startup` | Признак того, что инстанс успешно загрузился | `200`/`503`  | не требуется |
|  `/api/v1/health/live`   |  Признак того, что инстанс доступен в сети   | всегда `200` | не требуется |
|  `/api/v1/health/ready`  |   Признак того, что инстанс готов к работе   | `200`/`503`  | не требуется |


!!! note "Примечание"
    По умолчанию, данные эндпоинты в Picodata включены
    и доступны без дополнительной настройки. В [файле конфигурации] можно
    явным образом отключить их, указав параметр
    [`instance.http.kubernetes_probes`] со значением `false`.
...
- [состояние инстанса] оценивается как `Online`
- [Raft-лидер] в кластере определён
- репликасет, в котором состоит инстанс, [наполнен] (в таблице `_pico_replicaset`
в столбце `state` для него должно быть указано `ready`).

Для успешного ответа при обращении к эндпоинту `/ready` достаточно первых двух
критериев.

Согласно спецификации Kubernetes, эндпоинты `/ready` и `/live` будут отдавать
валидное состояние только после того, как `/startup` вернул код `200`. Иными
словами, нет смысла запрашивать `/ready` или `/live` до того как инстанс успешно
запустится и `/startup` вернёт код `200`.

[состояние инстанса]: ../tutorial/deploy.md#check_status
[Raft-лидер]: ../overview/glossary.md#raft_leader
[наполнен]: ../tutorial/node_add.md#check

Пример положительного ответа на запрос состояния:
...
```json
Failure (503):
{"status": "not_ready", "reason": "instance state is Offline"}
```
