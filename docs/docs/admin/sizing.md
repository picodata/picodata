...
Полученный объем относится к **пользовательскому** аллокатору `memtx`
(параметр [`instance.memtx.memory`]). Для [системных таблиц](../architecture/memtx_allocator.md)
кластера используется отдельный аллокатор с собственной квотой
[`instance.memtx.system_memory`] (по умолчанию 256 МБ); ее также необходимо
прибавлять к итоговому объему оперативной памяти, выделяемой на инстанс.
...
- `KOS` — коэффициент для ОС и кеша (1,2)
