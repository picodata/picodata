...
- исключить проблемный инстанс из кластера: `picodata expel <UUID> --peer <любой работающий инстанс>`
...
## Невозможность добавления новых инстансов в кластер {: #instance_peer_failure }
...
## Сообщение об ошибке повторного подключения к репликасету {: #instance_restart_error_messages }

**Описание**

Когда инстанс перезапускается и заново подключается к мастер-реплике
своего репликасета, в журнале сообщений инстанса можно увидеть сообщение
об ошибке следующего вида:

```shell
localhost [3369449] main/392/applier/pico_service@192.169.1.7:3190 box.cc:4483 E> ER_CFG: Incorrect value for option 'replication': duplicate connection with the same replica UUID
```

**Причина**

Сообщение появляется из-за того, что инстанс пытается переподключиться к
репликасету до того, как его мастер-реплика обработает предыдущую
попытку подключения.

**Решение**

Сообщение о дубликате подключения не является ошибкой и может быть проигнорировано.
