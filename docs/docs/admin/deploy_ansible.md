...
В данном разделе приведена информация по развёртыванию кластера Picodata
...
- имени тира (например `arbiter`)
- количества инстансов на каждом сервере (`instances_per_server`) или количества репликасетов (`replicaset_count`)
...
Более подробно о доступных переменных в инвентарном файле можно узнать
в статье [Переменные, используемые в роли Ansible](../reference/ansible_variables.md)
...
- [Документация в git-репозитории роли](https://git.picodata.io/core/picodata-ansible/-/blob/main/docs/).
...
## Добавление новых инстансов в кластер на существующих серверах {: #expand_cluster }
...
## Ребутстрап инстанса {: #rebootstrap_instance }

Применяется в случае выхода инстанса из строя и невозможности его ввода в репликасет кластера в обычном порядке.

!!! warning title "Внимание!"
    Проводить ребутстрап инстанса можно только в репликасетах с фактором репликации 2 и более,
    при этом инстанс не должен быть последним работающим в репликасете
...
!!! note "Примечание"
    С помощью роли Ansible можно только добавлять и обновлять
    плагины в кластере. Удаление производится
