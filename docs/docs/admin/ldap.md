...
помощью протокола LDAP, в том числе, с поддержкой шифрования StartTLS.
...
- LDAP + StartTLS: изначальное подключение выполняется к обычному порту
  ...
  шифрование
- LDAPS (Lightweight Directory Access Protocol Secure) — защищенная
  версия протокола LDAP. Подключение выполняется к отдельному порту
  (`636`), и полностью инкапсулируется внутри TLS/SSL. В этом случае
  никакого незашифрованного LDAP трафика по каналу не проходит.

Оба варианта обеспечивают конфиденциальность и безопасность
данных при доступе к службам каталогам.

Для использования LDAP и StartTLS при аутентификации в Picodata применяется TLS
...
Для использования LDAPS в строке подключения следует указать префикс
`ldaps://` и при этом не выставлять переменную `TT_LDAP_ENABLE_TLS`.
...
- наличие в сети запущенного сервера LDAP, на котором заведен этот
  пользователь
- объявление переменных окружения с данными сервера LDAP [при запуске узлов
...
Определение переменных окружения для использования сервера LDAP + StartTLS
