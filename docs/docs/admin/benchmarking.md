...
- адрес подключения. В примере выше используется инстанс, принимающий подключения по протоколу PostgreSQL по адресу `127.0.0.1:4327`. Убедитесь, что в вашем случае указано актуальное значение этого параметра (см. [instance.pgproto.listen])
