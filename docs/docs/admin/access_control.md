...
- `guest` — неавторизованный пользователь. У этого пользователя по
  умолчанию отсутствует пароль и привилегия `LOGIN`, поэтому подключиться
  от его имени через `psql` невозможно
...
- cert (аутентификация по клиентскому сертификату, см. [Аутентификация по
  сертификату](ssl.md#pgproto_cert_auth))

Для методов `md5`, `chap-sha1` и `scram-sha256` действуют
[требования](#allowed_passwords). Для методов `ldap` и `cert` пароль не
требуется и игнорируется.

!!! note "Примечание"
    При подключении по [протоколу PostgreSQL](../tutorial/connecting.md#postgresql)
    (например, через `psql`) поддерживаются только методы `md5`,
    `scram-sha256`, `ldap` и `cert`. Метод `chap-sha1` **не
    поддерживается** для этого протокола. По умолчанию новые пользователи
    создаются с методом `md5`.
...
!!! note "Требования к паролю"
    Для методов аутентификации `md5`, `chap-sha1` и `scram-sha256` пароль
    ...
    Подробнее см. [ниже](#allowed_passwords).
