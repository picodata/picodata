...
!!! note "Примечание"
    ...
    [instance.pgproto.tls.enabled], то небезопасные соединения (без TLS/SSL) при этом
...
[instance.pgproto.tls.enabled]: ../reference/config.md#instance_pgproto_tls
...
- односторонняя проверка сервера клиентом (режим TLS/SSL). Требуется пара
  сертификат + закрытый ключ на сервере (`instance.pgproto.ssl.cert_file` и
  `instance.pgproto.ssl.key_file`) и корневой сертификат на клиенте (`ca.crt`)
- двусторонняя проверка сервера и клиента друг другом (режим mTLS). На обеих
  сторонах нужны по 3 файла: `instance.pgproto.ssl.cert_file`,
  `instance.pgproto.ssl.key_file` и `instance.pgproto.ssl.ca_file` на сервере, и
  `client.crt`, `client.key` и `ca.crt` на клиенте. В результате
  PostgreSQL-сервер в Picodata будет принимать подключения только в том случае,
  если клиент предоставит сертификат, подписанный с помощью такого же `ca.crt`,
  у которого поле CN совпадает с именем пользователя (см. [Аутентификация по
  сертификату](#pgproto_cert_auth)).

Для включения [TLS/SSL] на стороне Picodata установите в [файле
конфигурации](../reference/config.md#instance_pgproto_tls) для параметра
`instance.pgproto.tls.enabled` значение `true` и положите на сервер
сертификат и ключ. Укажите пути к ним через параметры командной строки
или конфигурационный файл. Пример:
...
параметр `instance.pgproto.tls.ca_file` с путем до CA-сертификата, который
...
Picodata (параметр `instance.pgproto.tls.cert_file`). По умолчанию, `psql` будет
...
`pgproto.ssl.ca_file`, настроенного в Picodata.
...
параметра `instance.iproto.advertise`.
...
!!! note "Примечание"
    Если в `instance.iproto.advertise` указывается
...
(`instance.iproto.tls.cert_file`):
    ...
    - Поле `subjectAltName` сертификата `instance.iproto.tls.cert_file` должно
...
- [Настройка HTTPS в файле конфигурации](../reference/config.md#instance_http_tls)
