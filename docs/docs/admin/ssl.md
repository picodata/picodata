...
!!! note "Примечание"
    ...
    [instance.pg.tls.enabled], то небезопасные соединения (без TLS/SSL) при этом
...
[instance.pg.tls.enabled]: ../reference/config.md#instance_pg_tls
...
конфигурации](../reference/config.md#instance_pg_tls) для параметра
`instance.pg.tls.enabled` значение `true` и положите на сервер
...
параметр `instance.pg.tls.ca_file` с путем до CA-сертификата, который
...
Picodata (параметр `instance.pg.tls.cert_file`). По умолчанию, `psql` будет
