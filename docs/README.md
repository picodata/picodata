...
* [Тестирование документации Picodata](#тестирование-документации-picodata)
    ...
    * [Получение документации в формате PDF](#получение-документации-в-формате-pdf)
...
Этот способ автоматизирует генерацию PDF с помощью специального Python-скрипта `pdfmaker`, который находится в одноимённой директории. Зависимости скрипта включены в общий Pipfile и скорее всего уже установлены у вас.
...
Положите скрипт `pdfmaker` в директорию build.

Соберите руководство оператора:


```shell
./pdfmaker -o tutorial --pdf operator_guide.pdf
```

Соберите руководство администратора:


```shell
./pdfmaker -a admin --pdf admin_guide.pdf
```

Более подробное описание скрипта и его параметров находится в [pdfmaker/README.md](pdfmaker/README.md)

### Вручную

Это более старый способ, но он также неплохо работает.
