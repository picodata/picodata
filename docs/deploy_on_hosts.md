...
`picodata run --help`. С точки зрения внутренней архитектуры, [_кластер_](concepts.md#cluster)
...
См. [отдельный подраздел](../api) с описанием Lua API в Picodata. Дополнительно о внутренней архитектуре кластера Picodata можно узнать в разделе [Жизненный цикл кластера](../clustering).
