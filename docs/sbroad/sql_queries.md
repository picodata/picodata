...
...
```
Обязательными элементами плана запроса являются `projection` и `scan`.
Первый термин отвечает за выборку нужных столбцов из таблицы, второй — 
за сканирование таблицы. Построение проекции
(`projection`) всегда происходит перед сканированием. В рамках
...
Если в запросе есть условие (`where`), то в план добавляется элемент `selection`:
...
фильтрует данные по строкам/таплам (`ROW`).
