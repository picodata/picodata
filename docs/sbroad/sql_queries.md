...
- `DECIMAL`. Числовой тип данных. Позволяет хранить дробные числа с
  десятичным смещением знака дробной части (например, число `12.75`
  будет храниться как `1275` и смещением `2`);
- `DOUBLE` Числовой тип данных, вдвое более точный чем `float`.
  Позволяет хранить дробные числа и оперировать ими с более высокой
  точностью;
- `INT`, `INTEGER`. Тип целых чисел. Дробная часть не хранится и/или
  отбрасывается;
- `NUMBER`. Числовой тип данных. Значения хранятся в формате 64-битного
  числа двойной точности с плавающей запятой;
- `SCALAR`. Скалярный тип данных, т.е содержащий только один тип
  одновременно;
...
Для этого удобно подходит таблица с данными участников соревнования. В обычном виде она выглядит так:
```
...
```
Использование `CAST()` влияет на возможности манипуляции данными, а
также на результаты вычислений. Так, сумма баллов участников будет
отличаться в зависимости от выбранного типа данных.

Исходный тип `decimal`:
```
sbroad.execute([[select sum("score") from "scoring"]], {})
---
- {'metadata': [{'name': 'COL_1', 'type': 'decimal'}], 'rows': [[210.22]]}
...
```
Заданный тип `int`:
```
 sbroad.execute([[select sum(cast("score" as int)) from "scoring"]], {})
---
- {'metadata': [{'name': 'COL_1', 'type': 'decimal'}], 'rows': [[209]]}
...
```
По умолчанию Sbroad автоматически изменит название выводимого столбца (в
примерах выше это `COL_1`). Его можно явно переопределить, указав
собственное название. Оно не должно начинаться с цифры и не может
содержать пробелов и специальных служебных символов (кроме знака
подчеркивания).
Пример:
```
sbroad.execute([[select sum(cast("score" as int)) as "_Total_score_1" from "scoring"]], {})
---
- {'metadata': [{'name': '_Total_score_1', 'type': 'decimal'}], 'rows': [[209]]}
...
```
