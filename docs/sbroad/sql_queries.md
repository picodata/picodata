...
sbroad.execute([[insert into "characters" ("id", "name", "year") values (?, ?, ?)]], {1, "Woody", 1995})
```
Распределенные запросы выполняются отдельно на каждом шарде, после чего
результаты соединяются (см. [подробнее](sql_review.md)). 
...
Так как в запросе имена колонок не могут дублироваться, то псевдонимы
должны быть уникальными. Пример:
...
таблицы). Перемещение данных происходит по следующей схеме:
