...
Распределенный запрос выполняется отдельно на
каждом шарде, после чего результаты соединяются (см.
[подробнее](sql_review.md)). 

Вариант перемещения данных зависит от ключей шардирования таблицы, а
также от типа запроса. Если запрос затрагивает столбцы, разделенные по
шардам, то перемещение данных требуется, если же столбцы не разделены
(полностью находятся на шардах), то перемещения не происходит.
Соответственно, если запрос затрагивает ключи шардирования (столбцы, по
которым таблица распределена), то перемещения нет. Ключ шардирования
задается в схеме данных и может содержать один или несколько столбцов,
например:
...
Под _ключом шардирования_ понимается полное содержание параметра `sharding_key`, т.е. _все_ его столбцы.

**Полное перемещение** означает, что требуется целиком переместить
таблицу в правой части запроса на все шарды, содержащие таблицу в левой
части (без этого результаты локальных запросов будут неверными). Такое
перемещение происходит при распределенном исключении `EXCEPT`, а также в
тех вариантах соединения (`JOIN`), где в обеих частях запроса не
использованы соответствующие ключи шардирования (наличие полной копии
присоединяемой таблицы на всех шардах нужно для того, чтобы распределенный
`JOIN` смог корректно соединить столбцы разных таблиц).

**Частичное перемещение** предполагает копирование не всей таблицы, а
лишь необходимых столбцов. Это характерно для `INSERT`, а также `JOIN` c
произвольными столбцами соединения в правой части равенства.
...
`UNION ALL`), перемещение данных не происходит. Также, как было указано
выше, перемещения нет, если запрос затрагивает столбцы, по которым
таблица распределена по шардам.
