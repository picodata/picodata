...
### Спейс (space)
**Space (спейс)** — пространство хранения данных в СУБД. В резидентных СУБД спейс является синонимом таблицы из реляционных СУБД. В Picodata есть следующие виды спейсов:
...
В схеме данных кластера для каждого спейса (таблицы) обязательно присутствует параметр распределения (_ключ шардирования_), состоящий из одной или нескольких колонок. Пример:
...
sharding_key:
    ...
    - name
```
Под _ключом шардирования_ понимается __полное__ содержание параметра `sharding_key`, т.е. все его столбцы.
