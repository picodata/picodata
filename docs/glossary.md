...
**Data Definition Language (DDL)**. Язык определения данных. Набор команд для управления таблицами в SQL (создание/удаление таблиц).
...
**Команда SQL**. Разновидность команд в интерактивной консоли Picodata, обернутая в Lua-функцию pico.sql(). Включает в себя SQL-запрос и (опционально) значения передаваемых параметров.

**Запрос SQL**. Запрос к базе данных Tarantool на языке SQL.

**План запроса**. Схема и последовательность шагов, требуемая для исполнения запроса на узле-маршрутизаторе.

**Проекция**. Вертикальный фильтр таблицы, т.е. инструмент извлечения необходимых столбцов из таблицы.

**Ключ шардирования**. Столбец, по которому таблица распределена между шардами.
