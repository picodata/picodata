-- TEST-MATRIX: pgproto-1rsX1, pgproto-2rsX1, iproto-2rsX1

-- TEST: test_join
-- SQL:
DROP TABLE IF EXISTS testing_space;
DROP TABLE IF EXISTS t;
DROP TABLE IF EXISTS gt;
CREATE TABLE testing_space ("id" int primary key, "name" string, "product_units" int);
CREATE TABLE t ("a" INT PRIMARY KEY, "b" INT, "c" INT);
CREATE TABLE gt ("a" INT PRIMARY KEY) DISTRIBUTED GLOBALLY;
insert into "testing_space" ("id", "name", "product_units") values
    (1, 'a', 1),
    (2, 'a', 1),
    (3, 'a', 2),
    (4, 'b', 1),
    (5, 'b', 2),
    (6, 'b', 3),
    (7, 'c', 4);
INSERT INTO t ("a", "b", "c") VALUES
    (1, 2, 3),
    (4, 5, 6),
    (7, 8, 9);
INSERT INTO gt ("a") VALUES (1), (2);

-- TEST: join1
-- SQL:
select *
          from
            "testing_space"
          join
            (select t1."id" as f, t2."id" as s
             from
                (select "id" from "testing_space") t1
             join
                (select "id" from "testing_space") t2
             on true
            )
          on "id" = f and "id" = s order by 1;
-- EXPECTED:
1, 'a', 1, 1, 1,
2, 'a', 1, 2, 2,
3, 'a', 2, 3, 3,
4, 'b', 1, 4, 4,
5, 'b', 2, 5, 5,
6, 'b', 3, 6, 6,
7, 'c', 4, 7, 7

-- TEST: test_join2
-- SQL:
SELECT * FROM t JOIN t AS t1 ON 1 IN (t1.b, t1.a, t1.c) ORDER BY 1;
-- EXPECTED:
1, 2, 3, 1, 2, 3,
4, 5, 6, 1, 2, 3, 
7, 8, 9, 1, 2, 3

-- TEST: test_join3
-- SQL:
SELECT * FROM t JOIN t AS t1 ON t1.a IN (t1.b, t1.a) ORDER BY 1;
-- EXPECTED:
1, 2, 3, 1, 2, 3,
1, 2, 3, 4, 5, 6,
1, 2, 3, 7, 8, 9,
4, 5, 6, 1, 2, 3,
4, 5, 6, 4, 5, 6,
4, 5, 6, 7, 8, 9,
7, 8, 9, 1, 2, 3,
7, 8, 9, 4, 5, 6,
7, 8, 9, 7, 8, 9

-- TEST: test_join4
-- SQL:
SELECT * FROM gt join gt as gt2 ON TRUE JOIN t ON TRUE;
-- UNORDERED:
1,	1,	1,	2,	3,
1,	1,	4,	5,	6,
1,	1,	7,	8,	9,
1,	2,	1,	2,	3,
1,	2,	4,	5,	6,
1,	2,	7,	8,	9,
2,	1,	1,	2,	3,
2,	1,	4,	5,	6,
2,	1,	7,	8,	9,
2,	2,	1,	2,	3,
2,	2,	4,	5,	6,
2,	2,	7,	8,	9


-- TEST: test_join5
-- SQL:
SELECT * FROM t JOIN gt ON true join gt as gt2 ON TRUE;
-- UNORDERED:
1,	2,	3,	1,	1,
1,	2,	3,	1,	2,
1,	2,	3,	2,	1,
1,	2,	3,	2,	2,
4,	5,	6,	1,	1,
4,	5,	6,	1,	2,
4,	5,	6,	2,	1,
4,	5,	6,	2,	2,
7,	8,	9,	1,	1,
7,	8,	9,	1,	2,
7,	8,	9,	2,	1,
7,	8,	9,	2,	2

-- TEST: test-check-condition-types-1
-- SQL:
with t(b) as (select true) select * from t t1 join t t2 on t1.b or false;
-- EXPECTED:
true, true

-- TEST: test-check-condition-types-2
-- SQL:
with t(b) as (select true) select * from t t1 join t t2 on t1.b or 1;
-- ERROR:
could not resolve operator overload for or\(bool, int\)

-- TEST: test-sq-in-join-condition
-- SQL:
with t(a) as (values (1), (2), (3)) select * from t t1 join t t2 on (select true) and t1.a = t2.a;
-- UNORDERED:
1, 1, 2, 2, 3, 3

-- TEST: test-join-output-reduced-1.0
-- SQL:
DROP TABLE IF EXISTS t1;
CREATE TABLE t1 (
	a int primary key,
	b int
);
INSERT INTO t1 VALUES (1, 1), (2, 2), (3, 3);
DROP TABLE IF EXISTS t2;
CREATE TABLE t2 (
	a int primary key,
	b int
);
INSERT INTO t2 VALUES (1, 1), (2, 2), (3, 3);

-- TEST: test-join-output-reduced-1.1
-- SQL:
EXPLAIN (RAW, FMT, BUCKETS, LOGICAL)
SELECT t1.a, ROW_NUMBER() OVER (PARTITION BY t1.b ORDER BY t1.a) AS rn
FROM t1 JOIN t2 ON t1.a = t2.a;
-- EXPECTED:
──────────────────────────────────────────────────────────────────────
 # Logical plan                                                       
──────────────────────────────────────────────────────────────────────
''
projection (
  a::int,
  row_number() over (partition by (b::int) order by (a::int asc) ) -> rn
)
  motion [policy: segment([ref(b)]), program: ReshardIfNeeded]
    projection (t1.a::int -> a, t1.b::int -> b)
      join on (t1.a::int = t2.a::int)
        scan t1
        scan t2
''
──────────────────────────────────────────────────────────────────────
 # Raw plan                                                           
──────────────────────────────────────────────────────────────────────
''
╭──────────────────────────╮
│ 1. Query (WHOLE STORAGE) │
╰──────────────────────────╯
''
SELECT "t1"."a", "t1"."b" FROM "t1" INNER JOIN "t2" ON "t1"."a" = "t2"."a"
''
plan:
    [0] SCAN TABLE t1 (~1048576 rows)
        [0] SEARCH TABLE t2 USING PRIMARY KEY (a=?) (~1 row)
''
buckets <= [1-3000]
''
╭─────────────────────────────────╮
│ 2. Query (DYN-FILTERED STORAGE) │
╰─────────────────────────────────╯
''
SELECT
  "COL_0" as "a",
  row_number () OVER (
    PARTITION BY
      "COL_1"
    ORDER BY
      "COL_0" ASC
  ) as "rn"
FROM
  (
    SELECT
      "COL_0",
      "COL_1"
    FROM
      "_tmp_3265374087349123476_1136"
  )
''
plan:
    [1] SCAN TABLE _tmp_3265374087349123476_1136 (~1048576 rows)
    [1] USE TEMP B-TREE FOR ORDER BY
    [0] SCAN SUBQUERY 1 (~1 row)
''
buckets <= [1-3000]
''
──────────────────────────────────────────────────────────────────────
 # Buckets                                                            
──────────────────────────────────────────────────────────────────────
''
buckets <= [1-3000]

-- TEST: test-cross-table-constant-conditions-0
-- SQL:
DROP TABLE IF EXISTS ct1;
DROP TABLE IF EXISTS ct2;
CREATE TABLE ct1 (a int primary key);
CREATE TABLE ct2 (b int primary key);
INSERT INTO ct1 VALUES (1);
INSERT INTO ct2 VALUES (2);

-- TEST: test-cross-table-constant-conditions-1
-- SQL:
SELECT * FROM ct1 JOIN ct2 ON a = 1 AND b = 2;
-- EXPECTED:
1, 2

-- TEST: test-cross-table-constant-conditions-2
-- SQL:
SELECT * FROM ct1 JOIN ct2 ON a IN (1, 3) AND b = 2;
-- EXPECTED:
1, 2

-- TEST: test-cross-table-constant-conditions-3
-- SQL:
SELECT * FROM ct2 JOIN ct1 ON a = 1 AND b = 2;
-- EXPECTED:
2, 1

-- TEST: test-cross-table-constant-conditions-4
-- SQL:
SELECT * FROM ct1 JOIN ct2 ON a = 1 AND b = 3;
-- EXPECTED:

-- TEST: explain-cross-table-constant-conditions-1
-- SKIP_FOR: 2rsX1
-- SQL:
EXPLAIN(RAW, BUCKETS) SELECT * FROM ct1 JOIN ct2 ON a = 1 AND b = 2;
-- EXPECTED:
──────────────────────────────────────────────────────────────────────
 # Raw plan                                                           
──────────────────────────────────────────────────────────────────────
''
╭──────────────────────────╮
│ 1. Query (WHOLE STORAGE) │
╰──────────────────────────╯
''
SELECT "ct2"."b", "ct2"."bucket_id" FROM "ct2"
''
plan:
    [0] SCAN TABLE ct2 (~1048576 rows)
''
buckets <= [1-3000]
''
╭────────────────────────────────────────╮
│ 2. Query (CONST-FILTERED STORAGE, 1/1) │
╰────────────────────────────────────────╯
''
SELECT "ct1"."a", "ct2"."COL_0" as "b" FROM "ct1" INNER JOIN ( SELECT "COL_0", "COL_1" FROM "_tmp_420925595864221820_0136" ) as "ct2" ON "ct1"."a" = CAST(1 AS int) and "ct2"."COL_0" = CAST(2 AS int)
''
plan:
    [0] SEARCH TABLE ct1 USING PRIMARY KEY (a=?) (~1 row)
        [0] SCAN TABLE _tmp_420925595864221820_0136 (~262144 rows)
''
buckets = [1934]
''
──────────────────────────────────────────────────────────────────────
 # Buckets                                                            
──────────────────────────────────────────────────────────────────────
''
buckets <= [1-3000]

-- TEST: explain-cross-table-constant-conditions-2
-- SKIP_FOR: 2rsX1
-- SQL:
EXPLAIN(RAW, BUCKETS) SELECT * FROM ct1 JOIN ct2 ON a IN (1, 3) AND b = 2;
-- EXPECTED:
──────────────────────────────────────────────────────────────────────
 # Raw plan                                                           
──────────────────────────────────────────────────────────────────────
''
╭──────────────────────────╮
│ 1. Query (WHOLE STORAGE) │
╰──────────────────────────╯
''
SELECT "ct2"."b", "ct2"."bucket_id" FROM "ct2"
''
plan:
    [0] SCAN TABLE ct2 (~1048576 rows)
''
buckets <= [1-3000]
''
╭────────────────────────────────────────╮
│ 2. Query (CONST-FILTERED STORAGE, 1/1) │
╰────────────────────────────────────────╯
''
SELECT "ct1"."a", "ct2"."COL_0" as "b" FROM "ct1" INNER JOIN ( SELECT "COL_0", "COL_1" FROM "_tmp_15129504151034212097_0136" ) as "ct2" ON "ct1"."a" in (CAST(1 AS int), CAST(3 AS int)) and "ct2"."COL_0" = CAST(2 AS int)
''
plan:
    [0] SEARCH TABLE ct1 USING PRIMARY KEY (a=?) (~2 rows)
    [0] EXECUTE LIST SUBQUERY 1
        [0] SCAN TABLE _tmp_15129504151034212097_0136 (~262144 rows)
''
buckets = [1934,1958]
''
──────────────────────────────────────────────────────────────────────
 # Buckets                                                            
──────────────────────────────────────────────────────────────────────
''
buckets <= [1-3000]

-- TEST: explain-cross-table-constant-conditions-3
-- SKIP_FOR: 2rsX1
-- SQL:
EXPLAIN(RAW, BUCKETS) SELECT * FROM ct2 JOIN ct1 ON a = 1 AND b = 2;
-- EXPECTED:
──────────────────────────────────────────────────────────────────────
 # Raw plan                                                           
──────────────────────────────────────────────────────────────────────
''
╭──────────────────────────╮
│ 1. Query (WHOLE STORAGE) │
╰──────────────────────────╯
''
SELECT "ct1"."a", "ct1"."bucket_id" FROM "ct1"
''
plan:
    [0] SCAN TABLE ct1 (~1048576 rows)
''
buckets <= [1-3000]
''
╭────────────────────────────────────────╮
│ 2. Query (CONST-FILTERED STORAGE, 1/1) │
╰────────────────────────────────────────╯
''
SELECT "ct2"."b", "ct1"."COL_0" as "a" FROM "ct2" INNER JOIN ( SELECT "COL_0", "COL_1" FROM "_tmp_8428584780562940373_0136" ) as "ct1" ON "ct1"."COL_0" = CAST(1 AS int) and "ct2"."b" = CAST(2 AS int)
''
plan:
    [0] SEARCH TABLE ct2 USING PRIMARY KEY (b=?) (~1 row)
        [0] SCAN TABLE _tmp_8428584780562940373_0136 (~262144 rows)
''
buckets = [1410]
''
──────────────────────────────────────────────────────────────────────
 # Buckets                                                            
──────────────────────────────────────────────────────────────────────
''
buckets <= [1-3000]

-- TEST: explain-cross-table-constant-conditions-4
-- SKIP_FOR: 2rsX1
-- SQL:
EXPLAIN(RAW, BUCKETS) SELECT * FROM ct1 JOIN ct2 ON a = 1 AND b = 3;
-- EXPECTED:
──────────────────────────────────────────────────────────────────────
 # Raw plan                                                           
──────────────────────────────────────────────────────────────────────
''
╭──────────────────────────╮
│ 1. Query (WHOLE STORAGE) │
╰──────────────────────────╯
''
SELECT "ct2"."b", "ct2"."bucket_id" FROM "ct2"
''
plan:
    [0] SCAN TABLE ct2 (~1048576 rows)
''
buckets <= [1-3000]
''
╭────────────────────────────────────────╮
│ 2. Query (CONST-FILTERED STORAGE, 1/1) │
╰────────────────────────────────────────╯
''
SELECT "ct1"."a", "ct2"."COL_0" as "b" FROM "ct1" INNER JOIN ( SELECT "COL_0", "COL_1" FROM "_tmp_420925595864221820_0136" ) as "ct2" ON "ct1"."a" = CAST(1 AS int) and "ct2"."COL_0" = CAST(3 AS int)
''
plan:
    [0] SEARCH TABLE ct1 USING PRIMARY KEY (a=?) (~1 row)
        [0] SCAN TABLE _tmp_420925595864221820_0136 (~262144 rows)
''
buckets = [1934]
''
──────────────────────────────────────────────────────────────────────
 # Buckets                                                            
──────────────────────────────────────────────────────────────────────
''
buckets <= [1-3000]
