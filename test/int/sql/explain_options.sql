...
╰───────────────────╯
''
SELECT "a", "b", "c", "a", "b", "c" FROM ( SELECT "COL_0" as "a", "COL_1" as "b", "COL_2" as "c", "COL_3" as "a", "COL_4" as "b", "COL_5" as "c" FROM ( SELECT "COL_0", "COL_1", "COL_2", "COL_3", "COL_4", "COL_5" FROM "_tmp_4733185390354740506_1136" ) GROUP BY "COL_0", "COL_1", "COL_2", "COL_3", "COL_4", "COL_5" ) ORDER BY 4 LIMIT 5
''
plan:
    [0] SCAN TABLE _tmp_4733185390354740506_1136 (~1048576 rows)
    [0] USE TEMP B-TREE FOR GROUP BY
    [0] USE TEMP B-TREE FOR ORDER BY
''
buckets = any
...
FROM
  (
    ...
    SELECT
      ...
      "COL_2"
    FROM
      "_tmp_14378153340421086792_1136"
  )
ORDER BY
'  3'
LIMIT
...
plan:
    ...
    [3] SCAN TABLE _tmp_14378153340421086792_1136 (~1048576 rows)
  ...
  (
    (
      SELECT
        ...
        "COL_2"
      FROM
        "_tmp_17664831543678123679_2136"
    )
  )
ORDER BY
'  3'
...
''
plan:
    [0] SCAN TABLE _tmp_17664831543678123679_2136 (~1048576 rows)
    [0] USE TEMP B-TREE FOR ORDER BY
''
buckets = any
''
...
╰─────────────────────────────────╯
''
SELECT "a" FROM ( SELECT "t"."a" FROM "t" UNION ALL SELECT "COL_0" FROM "_tmp_2361693521261117575_1136" ) ORDER BY 1 LIMIT 1
''
plan:
    [1] SCAN TABLE t (~1048576 rows)
    [1] USE TEMP B-TREE FOR ORDER BY
    [2] SCAN TABLE _tmp_2361693521261117575_1136 (~1048576 rows)
...
╰───────────────────╯
''
SELECT "COL_0" as "a" FROM ( SELECT "COL_0" FROM "_tmp_11311034520916309730_2136" ) ORDER BY 1 LIMIT 1
''
plan:
    [0] SCAN TABLE _tmp_11311034520916309730_2136 (~1048576 rows)
    [0] USE TEMP B-TREE FOR ORDER BY
''
buckets = any
''
...
''
plan:
    [0] SCAN TABLE _tmp_1342503674006629704_1136 (~1048576 rows)
    [0] USE TEMP B-TREE FOR GROUP BY
''
buckets = any
''
...
WHERE
  "tt"."d" in (
    SELECT
      "COL_0"
    FROM
      "_tmp_15666534998603130372_1136"
  )
''
plan:
    [0] SEARCH TABLE tt USING PRIMARY KEY (d=?) (~24 rows)
    [0] EXECUTE LIST SUBQUERY 1
    [1] SCAN TABLE _tmp_15666534998603130372_1136 (~1048576 rows)
''
buckets <= [1-3000]
''
──────────────────────────────────────────────────────────────────────
...
FROM
  (
    SELECT
      "COL_0"
    FROM
      "_tmp_9764198614563956109_1136"
  )
ORDER BY
'  1'
''
plan:
    [0] SCAN TABLE _tmp_9764198614563956109_1136 (~1048576 rows)
    [0] USE TEMP B-TREE FOR ORDER BY
''
buckets = any
''
...
WHERE
  ...
  and "tt"."d" = (
    SELECT
      "COL_0"
    FROM
      "_tmp_6222629172308997546_1136"
  )
''
plan:
    [0] SEARCH TABLE tt USING PRIMARY KEY (d=?) (~1 row)
    [0] EXECUTE SCALAR SUBQUERY 1
    [1] SCAN TABLE _tmp_6222629172308997546_1136 (~1048576 rows)
...
WHERE
  ...
  and "tt"."d" = (
    SELECT
      "COL_0"
    FROM
      "_tmp_7331740932410388311_1136"
  )
''
plan:
    [0] SEARCH TABLE tt USING PRIMARY KEY (d=?) (~1 row)
    [0] EXECUTE SCALAR SUBQUERY 1
    [1] SCAN TABLE _tmp_7331740932410388311_1136 (~1048576 rows)
''
buckets <= [1-3000]
''
──────────────────────────────────────────────────────────────────────
