...
╰───────────────────╯
''
SELECT "a", "b", "c", "a", "b", "c" FROM ( SELECT "COL_0" as "a", "COL_1" as "b", "COL_2" as "c", "COL_3" as "a", "COL_4" as "b", "COL_5" as "c" FROM ( SELECT "COL_0", "COL_1", "COL_2", "COL_3", "COL_4", "COL_5" FROM "_tmp_5392829762195339068_1136" ) GROUP BY "COL_0", "COL_1", "COL_2", "COL_3", "COL_4", "COL_5" ) ORDER BY 4 LIMIT 5
''
plan:
    [0] SCAN TABLE _tmp_5392829762195339068_1136 (~1048576 rows)
    [0] USE TEMP B-TREE FOR GROUP BY
    [0] USE TEMP B-TREE FOR ORDER BY
''
buckets = any
...
plan:
    ...
    [3] SCAN TABLE _tmp_8016438503807243129_1136 (~1048576 rows)
...
╰─────────────────────────────────╯
''
SELECT "a" FROM ( SELECT "t"."a" FROM "t" UNION ALL SELECT "COL_0" FROM "_tmp_1606600361159225431_1136" ) ORDER BY 1 LIMIT 1
''
plan:
    [1] SCAN TABLE t (~1048576 rows)
    [1] USE TEMP B-TREE FOR ORDER BY
    [2] SCAN TABLE _tmp_1606600361159225431_1136 (~1048576 rows)
...
╰───────────────────╯
''
SELECT "COL_0" as "a" FROM ( SELECT "COL_0" FROM "_tmp_9370252247648176216_2136" ) ORDER BY 1 LIMIT 1
''
plan:
    [0] SCAN TABLE _tmp_9370252247648176216_2136 (~1048576 rows)
    [0] USE TEMP B-TREE FOR ORDER BY
''
buckets = any
''
...
WHERE
  "tt"."d" in (
    SELECT
      "COL_0"
    FROM
      "_tmp_3764215175587176145_1136"
  )
''
plan:
    [0] SEARCH TABLE tt USING PRIMARY KEY (d=?) (~24 rows)
    [0] EXECUTE LIST SUBQUERY 1
    [1] SCAN TABLE _tmp_3764215175587176145_1136 (~1048576 rows)
''
buckets <= [1-3000]
''
──────────────────────────────────────────────────────────────────────
...
WHERE
  ...
  and "tt"."d" = (
    SELECT
      "COL_0"
    FROM
      "_tmp_7182777534507920409_1136"
  )
''
plan:
    [0] SEARCH TABLE tt USING PRIMARY KEY (d=?) (~1 row)
    [0] EXECUTE SCALAR SUBQUERY 1
    [1] SCAN TABLE _tmp_7182777534507920409_1136 (~1048576 rows)
...
WHERE
  ...
  and "tt"."d" = (
    SELECT
      "COL_0"
    FROM
      "_tmp_1858486628017346301_1136"
  )
''
plan:
    [0] SEARCH TABLE tt USING PRIMARY KEY (d=?) (~1 row)
    [0] EXECUTE SCALAR SUBQUERY 1
    [1] SCAN TABLE _tmp_1858486628017346301_1136 (~1048576 rows)
''
buckets <= [1-3000]
''
──────────────────────────────────────────────────────────────────────
