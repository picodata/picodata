...
-- TEST: test_query_plan_explain-1
-- SQL:
EXPLAIN (QUERY PLAN) SELECT * from testing_space WHERE "id" = 1;
-- EXPECTED:
projection (case when true::bool then '1'::string else TRIM('2'::string) end -> "col_1")
execution options:
    sql_vdbe_opcode_max = 45000
...
-- TEST: test_query_plan_explain-2
-- SQL:
EXPLAIN (QUERY PLAN) SELECT * from testing_space WHERE "id" = 1 ORDER BY 1 LIMIT 1;
-- EXPECTED:
projection (case when true::bool then '1'::string else TRIM('2'::string) end -> "col_1")
execution options:
    sql_vdbe_opcode_max = 45000
...
-- TEST: test_query_plan_explain-3
-- SQL:
EXPLAIN (QUERY PLAN) SELECT * from testing_space WHERE "id" = 1 GROUP BY 1, 2, 3 ORDER BY 1 LIMIT 1;
-- EXPECTED:
- - SELECT "testing_space"."id" as "gr_expr_1", "testing_space"."name" as "gr_expr_2",
    "testing_space"."product_units" as "gr_expr_3" FROM "testing_space" WHERE "testing_space"."id"
    = CAST($1 AS int) GROUP BY "testing_space"."id", "testing_space"."name", "testing_space"."product_units"
  - +----------+-------+------+--------------------------------------------------------------+
  - '| selectid | order | from | detail                                                       |'
  - +========================================================================================+
  - '| 0        | 0     | 0    | SEARCH TABLE testing_space USING PRIMARY KEY (id=?)
    (~1 row) |'
  - +----------+-------+------+--------------------------------------------------------------+
  - 
  - 
  - SELECT "id", "name", "product_units" FROM (SELECT "COL_0" as "id", "COL_1" as
    "name", "COL_2" as "product_units" FROM (SELECT "COL_0","COL_1","COL_2" FROM "TMP_11019145509151764786_0136")
    GROUP BY "COL_0", "COL_1", "COL_2") ORDER BY 1 LIMIT 1
  - +----------+-------+------+----------------------------------------------------------+
  - '| selectid | order | from | detail                                                   |'
  - +====================================================================================+
  - '| 0        | 0     | 0    | SCAN TABLE TMP_11019145509151764786_0136 (~1048576
    rows) |'
  - '|----------+-------+------+----------------------------------------------------------|'
