...
def test_discovery(cluster3: Cluster):
    ...
    Retriable().call(i1.assert_raft_status, "Follower", i2.raft_id)
...
def test_basic_replication_setup(cluster: Cluster):
    ...
    for instance in cluster.instances:
        ...
        Retriable().call(check_replicated, instance)
    ...
    i1.restart()
    Retriable().call(check_replicated, i1)
...
def test_concurrent_join_does_not_fail_with_bootstrap_connection_error(cluster: Cluster):
    error_injection_for_all = "PICODATA_ERROR_INJECTION_USE_SHORT_REPLICATION_CONNECT_TIMEOUT"
    ...
    lagger = cluster.add_instance(name="lagger", wait_online=False, replicaset_name=leader.replicaset_name)
