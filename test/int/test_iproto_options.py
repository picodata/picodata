...
def test_invalid_parametrized_sql_options(instance: Instance):
    ...
    with pytest.raises(
        TarantoolError,
        match=r"""invalid query: expected 1 values for parameters, got 0""",
    ):
        instance.sql(q1)
    ...
    with pytest.raises(
        TarantoolError,
        match=r"""invalid OptionSpec: expected option sql_motion_row_max to be a non-negative integer, got: String\("1"\)""",
    ):
        # passing a string for an integer doesn't work, even if it's a valid textual representation
        instance.sql(q1, "1")
    with pytest.raises(
        TarantoolError,
        match=r"""invalid OptionSpec: expected option sql_motion_row_max to be a non-negative integer, got: String\("много"\)""",
    ):
        # a string straight up also doesn't work
        instance.sql(q1, "много")
    with pytest.raises(
        TarantoolError,
        match=r"invalid OptionSpec: expected option sql_motion_row_max to be a non-negative integer, got: Integer\(-1\)",
    ):
