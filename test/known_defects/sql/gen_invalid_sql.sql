...
╰───────────────────╯
''
SELECT "COL_0" FROM "_tmp_10778552413633097810_0136" LIMIT 1
''
plan:
    [0] SCAN TABLE _tmp_10778552413633097810_0136 (~1048576 rows)
''
╭───────────────────╮
│ 4. Query (ROUTER) │
╰───────────────────╯
...
╰──────────────────────────╯
''
SELECT CAST(1 AS int) as "col_1" FROM "t" INNER JOIN ( SELECT "COL_0" FROM "_tmp_18377525730552408791_2136" ) as "a" ON ( SELECT "COL_0" FROM "_tmp_18377525730552408791_3136" )
''
plan:
    [0] SCAN TABLE t (~1048576 rows)
    [0] EXECUTE SCALAR SUBQUERY 1
    [1] SCAN TABLE _tmp_18377525730552408791_3136 (~1048576 rows)
        [0] SCAN TABLE _tmp_18377525730552408791_2136 (~1048576 rows)
